...
{$IFEND}
    ...
    procedure ClientGetSendHeroItems(body: string); //从服务端获取英雄物品ID     $001
...
{$IFEND}
    ...
    //发送给登陆网关的第一个包 By TasNat at: 2012-03-24 13:51:52
...
var
  ...
  MP3: TMPEG;
  Video: TMPEG;
  g_WlMinute : Integer = $F0F0FF0A;//从WL获取时间
...
{$IF GVersion <> 1}
  ...
  OldOpenFileMapping: THelperHu1; //反外挂内存共享
  //钩新线程创建 如果开始地址不在允许的dll内则屏蔽  By TasNat at: 2012-05-11 13:18:04
...
begin
  Result := mrOk;
{$IF GVersion = 0} //当为测试版时 调用 LoginDlg
...
begin
  Result := 0;
  Result := OldLoadLibrary(lpLibFileName);Exit;//开启后输入法切换就正常了.
...
begin
  ...
  (UpperCase(FindHInstance(Ret)) = SetDate('XFAFAJ[!KCC')) or {WININET.DLL}
    //(AnsiStrPos(lpName, PChar(SetDate('B\'))) <> nil) or//如果有问题把这行注释去掉 By TasNat at: 2012-04-02 11:01:22
...
var
  boCode: Boolean;
begin
  //修改 By TasNat at: 2012-03-31 14:02:55
...
{$IFEND}
  //动态创建反加速 By TasNat at: 2012-03-10 16:32:17
  with TTimer.Create(Self) do begin
    Interval := 6 * 1000; //6秒执行一次
    OnTimer := TimerCheckSpeedTimer;
  end;

  //动态创建反加速通过WL的试用时间来分析GetTickCount的时间是否正确以达到封加速的目的 By TasNat at: 2012-10-12 11:32:17
  {with TTimer.Create(Self) do begin
    Interval := 2 * 60 * 1000; //2分钟执行一次
    OnTimer := TimerCheckSpeedTimerWL;
  end; }
  g_WlMinute := WLTrialRuntimeLeft xor $F0F0FF0A;
  g_TickMinute := (GetTickCount div 1000) xor $F0F0FF0A;
  g_WilList := TList.Create; //增加所有资源文件列表 处理D3D 设备丢失 By TasNat at: 2012-03-12 16:02:17
  LoadWMImagesLib(nil);
  ...
  try
    ...
    on E: Exception do
      ShowMessage(E.Message + sLineBreak + '没有检测到你机器的声卡驱动，请安装声卡驱动!');
  end;
...
{$IF GVersion <> 1}
  ...
  //@OldCreateThread := HelperHCode(GetProcAddress(GetModuleHandle(kernel32), PChar(SetDate('L}jn{j[g}jnk' {CreateThread}))), @HookCreateThread);
    //@OldOpenFileMapping := HelperHCode(GetProcAddress(GetModuleHandle(kernel32), PChar(SetDate('@jaIfcjBnfahN'{OpenFileMappingA}))), @HookOpenFileMappingA); //反外挂内存共享
...
{$IFEND}
  FillChar(g_BoxsItems, sizeof(TClientItem) * 12, #0); //释放宝箱物品
...
{$IFEND}
  ...
  g_CommandList.DelimitedText := '加入门派|退出门派|-|设置允许收徒|设置拒绝收徒|-|设置拒绝求婚|设置允许求婚|-|使用天地合一|开关天地合一|-|普通说话|黄字喊话(!)|行会聊天(!~)|组队聊天(!!)|私聊(/)|千里传音';
  //CSocket.Active:=True;
  //DebugOutStr ('----------------------- started ------------------------');
  ...
  g_GameDevice.VSync := g_D3DConfig.boVSync; //和显卡FPS同步
...
{$IF GVersion <> 1}
  ...
  UnHelperHCode(@OldOpenFileMapping); //反外挂内存共享
...
{$IFEND}
    ...
    VK_F12, VK_HOME: begin //TasNat 防止调试出现 CPU 窗口2012-03-03 21:21:27
        Key := 0;
        OpenSdoAssistant();
      end;
    ...
    //这里是获取不到Tab事件的 By TasNat at: 2012-04-02 15:38:41
    {VK_TAB: begin //切换小地图
...
{$IFEND}
    ...
    word('X'):
      begin
        if g_MySelf = nil then Exit;
{$IF M2Version <> 2}
          //TasNat 2012-03-04 11:15:49 灵煤快捷键需要在聊天输入框不显示的状态下才有效，防止和剪切键冲突
        ...
        if ssAlt in Shift then begin
          if (GetTickCount - g_dwQueryItems > 5000) and (not g_MySelf.m_boDeath) then begin
            ...
            if FrmDlg.DItemBag.Visible then begin
              ...
              g_dwQueryItems := GetTickCount(); //修正刷新包裹提示错误 By TasNat at: 2012-04-07 13:41:12
...
begin
  ...
  if ((g_MySelf.m_nState and $04000000 <> 0) {麻痹} and (pcm.Def.wMagicId <> 102)) or g_MySelf.m_boIsShop or (g_MySelf.m_nState and $1000000 <> 0) {冰冻} then Exit;


  if (tx = 5000) and (ty = 5000) then begin //放合击
    g_dwMagicDelayTime := 200; //修正放合击同时放技能会卡住
    g_dwLatestSpellTick := GetTickCount;
    msgs := MakeDefaultMsg(aa(CM_HEROGOTETHERUSESPELL, TempCertification), 0, 0, 0, 0, m_nSendMsgCount);
    FrmMain.SendSocket(EncodeMessage(msgs));
  end else begin
    // add by liuzhigang 使用了龙卫心法，就不能再使用无极真气。这地方也说明了，所有的执行动作，需得到服务器认可才能执行。
    {if (pcm.Def.wMagicId=50) then
    begin
      if g_MySelf.m_nState and $00020000 <> 0 then Exit; //心法激活
      for I:=0 to PlayScene.m_EffectList.Count-1 do //三焰咒
        if TMagicEff(PlayScene.m_EffectList[I]).EffectBase=110 then Exit;
    end;}
    if pcm.Def.wMagicId = 105 then
    begin //心法  Copy by liuzhigang on 2011.11.18
    ...
    //魔法锁定
        if (pcm.Def.wMagicId in [2, 14, 15, 19, 9, 10, 22, 23, 29, 31, 33, 46, 49, 40, 52, 56..58]) then //此处为不锁定
        ...
        if g_MagicTarget = nil then begin
          ...
          //使用服务端发来的最新坐标By TasNat at: 2012-07-22 13:47:02
...
{$IF M2Version <> 2}
                                      ...
                                      end; //纵横剑术 By TasNat at: 2012-03-07 14:14:35
    ...
    g_dwLastAttackTick := GetTickCount;
     ...
     //非近身 By TasNat at : 2012-03-04 18:15:57
...
{$IF M2Version <> 2}
                ...
                end else //g_boPosLongHit 是个开关 由 DCheckSdoPosLongHit 控制
                    if (*{$IF M2Version = 2}g_boPosLongHit and {$IFEnd}*) g_boCanLongHit and (TargetInSwordLongAttackRange(tdir)) and not ClientConf.boNoCanUseComparThrust then begin //隔位刺杀
                      if CanNextAction and ServerAcceptNextAction and CanNextHit then begin
                        if g_boCan4LongHit then begin
                          if GetMagicEffLevelEx(89) <> 0 then begin
                            btEffectLevelEx := GetMagicEffLevelEx(89);
...
begin
  Result := False;
  if (g_MySelf = nil) or (g_MySelf.m_boIsShop) or (g_MySelf.m_nState and $04000000 <> 0 {麻痹}) or (g_MySelf.m_nState and $1000000 <> 0 {冰冻}) then Exit;
...
//百分比吃药 By TasNat at: 2012-04-06 15:22:03

function NeedEat(nMax, nCur, nBai: Integer): Boolean;
begin
  Result := (nMax > 0); //防止除0错误
  Result := Result and (nBai < 100); //防止SB设置成100
...
//显示英雄的物品拿起时的外形
  ...
  //d := AspTextureFont.GetTextTexture('按时法');
  ...
  if not FrmDlg.DLOGO.Visible then begin
    if g_ConnectionStep = cnsLogin then begin
      AspTextureFont.TextOut((g_D3DConfig.wScreenWidth - 800) div 2 + 360, (g_D3DConfig.wScreenHeight - 600) div 2 + 535, '健康游戏公告', $0093F4F2);
      AspTextureFont.TextOut((g_D3DConfig.wScreenWidth - 800) div 2 + 190, (g_D3DConfig.wScreenHeight - 600) div 2 + 553, '抵制不良游戏，拒绝盗版游戏。注意自我保护，谨防受骗上当。适度游戏益脑，', $0093F4F2); //显示出logo文字
      AspTextureFont.TextOut((g_D3DConfig.wScreenWidth - 800) div 2 + 190, (g_D3DConfig.wScreenHeight - 600) div 2 + 571, '沉迷游戏伤身。合理安排游戏，享受健康生活。严厉打击赌博，营造和谐环境。', $0093F4F2); //显示出logo文字
{$IF M2Version = 0}
...
{$IF GVersion = 0}
    ...
    if str = '@p' then begin
       //DScreen.AddCenterLetter(1,0,3000, '哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈');
       {DScreen.AddTopLetter(251, 0,'哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈');
       DScreen.AddTopLetter(251, 0,'呵呵呵呵呵呵 呵呵呵呵');  }
       //DScreen.AddMoveMsg('你好啊，大沟施，', 251, 0, 0, 0, 1);
...
begin
  if (g_MySelf.IsIdle) and
    ((g_MySelf.m_nState and $04000000 = 0) {非麻痹}) and (g_MySelf.m_nState and $1000000 = 0) {非冰冻} and
  ...
  try
    ...
    if BufferStr <> '' then begin
      ...
      while (nTagCount > 0) and (not Application.Terminated) do begin
        ...
        DecodeMessagePacket(data);
        nTagCount := TagCount(BufferStr, '!');
            //防止封包过多而卡死  By TasNat at: 2012-03-14 20:54:25
...
begin
  ...
  if g_boFirstTime then begin
    FrmDlg.Initialize;//修复alt+回车 提示错误
    FrmDlg.InitializePlace;
    g_boFirstTime := FALSE;
...
begin
  new(pcm);
  DecodeBuffer(body, @(pcm^), sizeof(TClientMagic));
  if pcm.Def.wMagicId = 89 then g_boCan4LongHit := True; //4级刺杀剑术
  ...
  if g_MagicList.Count > 0 then //20080629
    for i := g_MagicList.Count - 1 downto 0 do begin
      if PTClientMagic(g_MagicList[i]).Def.wMagicId = magid then begin
        if magid = 89 then g_boCan4LongHit := False; //4级刺杀剑术
  ...
  with FrmDlg do begin
    ...
    if g_SaveItemList.Count > 0 then //20080629
      for i := 0 to g_SaveItemList.Count - 1 do begin
        ...
        if PTClientItem(g_SaveItemList[i]).s.StdMode = 17 then begin //叠加物品
...
// new copy by liuzhigang, 接受服务端的命令，启动魔法。
...
{$IF M2Version <> 2}
    26: begin // 龙卫心法的执行。
        if Actor <> nil then begin // 110 图片开始索引  40 播放多少张图片 60 每张图片(每帧)播放的时间.
...
{$IFEND}
  FrmDlg.DBotStall.Visible := ClientConf.boUsePlayShop; //是否开放个人商店 20100706
...
begin
  case Message.Active of
    WA_ACTIVE: begin
        ...
        if (I <> 0) then
        begin
          MessageBox(Handle, PChar('D3D初试化失败. Code:' + IntToStr(I) + '.' + IntToStr(AsphyreError)), PChar(g_sVersion), 0);
          Application.Terminate();
          Exit;
        end;
        end;
    ...
    WA_INACTIVE: begin
        ...
        begin
        //修复全屏TAB 切换 不修改分辨率 By TasNat at: 2012-03-31 13:27
          if not g_GameDevice.Windowed then begin
            ChangeDisplaySettings(lpDefDevMode, CDS_FULLSCREEN);
...
begin
  case Msg.CmdType of
    ...
    SC_RESTORE: begin
      //恢复分辨率 By TasNat at: 2012-03-31 13:27:41
        if not g_GameDevice.Windowed then begin
          ChangeDisplaySettings(lpDevMode, CDS_FULLSCREEN);
        end;
        boMinimized := False;
...
var
  nTick: Cardinal;
begin
  nTick := GetTickCount; //反刺影By TasNat at: 2012-05-14 10:25:25
  ...
  begin
    ...
    if nTick > 200 then begin //Sleep精度不行
      //Inc(g_SpeedCount);
      asm //关闭程序
      MOV FS:[0],0;
      MOV DS:[0],EAX;
...
var
   msg: TDefaultMessage;
begin
  if not CSocket.Socket.Connected or boSendOK then Exit;//防止多次发送导致弹多次选择服务器框By TasNat at: 2012-10-12 13:14:40
  boSendOK := True;
...
begin
  ...
  if {(GetTickCount - g_dwLatestStruckTick > 10000) and
    ...
    if mrOk = FrmDlg.DMessageDlg('是否确认退出游戏？', [mbOk, mbCancel]) then begin
      Application.Terminate; //防止出异退不了游戏 By TasNat at: 2012-03-11 14:31:46
...
begin
  ...
  ImageLogo.Repaint;
  Application.ProcessMessages;   }
  if not g_GameDevice.Active then begin //增加D3D状态检测By TasNat at: 2012-03-27 11:35:57
{$IF GVersion = 1}
    g_sTArr[21] := Char(108);
    g_sTArr[22] := Char(105);
    g_sTArr[23] := Char(99);
    ...
    if not g_GameDevice.Windowed then begin
      ...
      if ChangeDisplaySettings(lNeedDevMode, CDS_FULLSCREEN) <> DISP_CHANGE_SUCCESSFUL then begin
        if ChangeDisplaySettings(lNeedDevMode, CDS_UPDATEREGISTRY) <> DISP_CHANGE_SUCCESSFUL then begin
          //ShowMessage('修改桌面分辩率失败！' + SysErrorMessage(GetLastError));
          if ChangeDisplaySettings(lNeedDevMode, CDS_FULLSCREEN) <> DISP_CHANGE_SUCCESSFUL then begin
...
begin
  with FrmDlg do begin
    if m_btSignedItemsPage = 1 then begin
      //交换 By TasNat at: 2012-03-06 11:29:43
...
// wParam=Pos, Value nParam1=玩家或者英雄  nParam2:玩家的ID
  ...
  if body <> '' then begin
    ...
    with FrmDlg.DMemoXinFaHint.Lines do begin
      with g_HeartAbility do begin
        Clear;
        if not boHeartTpye then begin //传承
...
{$IFEND}
  ...
  feature.nDressLook := 65535; //衣服外观特效
  feature.nWeaponLook := 65535; //武器外观特效
  ...
  if g_MySelf = nil then begin
    case msg.Ident of
      ...
      SM_SENDLOGINKEY: begin
          ...
          WaitAndPass(500); //延迟发送  防止封内容连到一起
          g_boLOGINKEYOk := True;
        end;
      SM_NEWID_SUCCESS:
        begin
      ...
      SM_NEWID_FAIL:
        begin
          case msg.Recog of
            ...
            -3: FrmDlg.DMessageDlg('不允许帐号和密码相同!', [mbOk]);
          else
            FrmDlg.DMessageDlg('帐号无法创建，请不要用空格及非法字符注册 :  ' + IntToStr(msg.Recog), [mbOk]);
          end;
        end;
      SM_PASSWD_FAIL:
        begin
...
//获取验证码
      ...
      SM_CHECKNUM_OK: begin
          FrmDlg.DWCheckNum.Visible := False;
           //UiDXImageList.Items[35].Picture.Assign(nil); // 要调整
        end;
...
{$IF M2Version <> 2}
        ...
        SM_NewUSERKAMPO_OK: with g_SignedItem[0], FrmDlg do begin //鉴定物品成功
        ...
        SM_NewUSERKAMPO_FAIL: with FrmDlg do begin //鉴定物品失败
            DMessageDlg('鉴定物品失败.', [mbOK]);
            if not DCHSignedItemAutoLockValue.Checked then begin
              DCHSignedItemValue1.Checked := False;
              DCHSignedItemValue2.Checked := False;
              DCHSignedItemValue3.Checked := False;
...
{$IFEND}
      ...
      {SM_CLOSEDRAGONPOINT: begin
        ...
        SM_FIRDRAGONPOINT: if g_HeroSelf <> nil then begin //英雄怒气值
        ...
        SM_HEROBAGITEMS: if g_HeroSelf <> nil then begin //接收英雄包裹物品
        ...
        SM_SENDHEROUSEITEMS: if g_HeroSelf <> nil then begin //接收英雄身上装备
            ClientGetSendHeroItems(body);
          end;
        SM_HEROABILITY: if g_HeroSelf <> nil then begin //接收 英雄属性1
...
{$IFEND}
        ...
        SM_HEROSUBABILITY: if g_HeroSelf <> nil then begin //接收 英雄属性2
...
{$IFEND}
        ...
        SM_HEROTAKEOFF_OK: if g_HeroSelf <> nil then begin //返回英雄从装备到包裹成功   2007.10.24
            DecodeBuffer(DecodeString(body), @feature, sizeof(Tfeatures));
            g_HeroSelf.m_nFeature := feature; //msg.Recog;
            g_HeroSelf.FeatureChanged;
            g_WaitingHeroUseItem.Item.S.Name := '';
          ...
          begin
            ...
            if actor <> nil then begin
              actor.SendMsg(msg.Ident,
                ...
                0, 0, '',
                0, g_nilFeature);
              actor.m_nTargetX := Msgwl.lParam1; //x 带瘤绰 格钎
              actor.m_nTargetY := Msgwl.lParam2; //y
              actor.m_nTargetRecog := Msgwl.lTag1;
              actor.m_nMagicNum := Msgwl.lTag2; //付过 锅龋
            end;
          end;
