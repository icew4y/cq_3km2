...
begin
  ...
  if ListItem <> nil then begin
    ...
    g_D3DConfig.boFullScreen := not CheckBoxWindowMode.Checked;
    //inet_addr(PChar(Address))
    case ComboBoxScreenMode.ItemIndex of
      0:begin g_D3DConfig.wScreenWidth := 800; g_D3DConfig.wScreenHeight := 600; end;
      1:begin g_D3DConfig.wScreenWidth := 1024; g_D3DConfig.wScreenHeight := 768; end;
      2:begin g_D3DConfig.wScreenWidth := 900; g_D3DConfig.wScreenHeight := 600; end;
    end;
    if RadioGroupVer.ItemIndex < 0 then RadioGroupVer.ItemIndex := 3;
    //g_nClientVersion := RadioGroupVer.ItemIndex;
    g_D3DConfig.boVSync := CheckBoxVSync.Checked;
    g_D3DConfig.boD3DFormat := CheckBoxD3DFormat.Checked;
    if ComboBoxBitCount.ItemIndex = 0 then g_D3DConfig.btBitCount := 32 else g_D3DConfig.btBitCount := 16;
    Close;
    ModalResult := mrOk;
