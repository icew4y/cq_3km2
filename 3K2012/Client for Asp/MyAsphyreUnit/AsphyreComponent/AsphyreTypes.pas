...
//---------------------------------------------------------------------------
uses
  Types, Vectors2px, Vectors2, AsphyreColors, CommonDef, Graphics;
...
function pBounds4M(_Left, _Top, _Width, _Height, _MemoryWidth, _MemoryHeight: Single): TPoint4;
...
begin
  ...
  else Result := ((N and (N - 1)) = 0) and (N <> 0) and (N <> 1);
end;
...
begin
{$IFDEF VER185}
  Result := AspColorToCardinal(AspColorMultiply(CardinalToAspColor(Color), CardinalToAspColor(cAlpha1(Alpha))));
{$ELSE}
...
begin
{$IFDEF VER185}
  Result := AspColorToCardinal(AspColorMultiply(CardinalToAspColor(Color), cColor(Gray, Gray, Gray, Alpha)));
{$ELSE}
...
begin
{$IFDEF VER185}
  Result[0] := CardinalToAspColor(Colors[0]);
  Result[1] := CardinalToAspColor(Colors[1]);
  Result[2] := CardinalToAspColor(Colors[2]);
  Result[3] := CardinalToAspColor(Colors[3]);
{$ELSE}
  Result[0] := Colors[0];
  Result[1] := Colors[1];
  Result[2] := Colors[2];
...
begin
{$IFDEF VER185}
  Result[0] := AspColorToCardinal(Colors[0]);
  Result[1] := AspColorToCardinal(Colors[1]);
  Result[2] := AspColorToCardinal(Colors[2]);
  Result[3] := AspColorToCardinal(Colors[3]);
{$ELSE}
  Result[0] := Colors[0];
  Result[1] := Colors[1];
  Result[2] := Colors[2];
...
begin
  Result[0].X := _Left;
  Result[0].Y := _Top;
  Result[1].X := _Left + _MemoryWidth;
  Result[1].Y := _Top;
  Result[2].X := _Left + _Width;
  Result[2].Y := _Top + _MemoryHeight;
  Result[3].X := _Left;
  Result[3].Y := _Top + _Height;
end;
//---------------------------------------------------------------------------
