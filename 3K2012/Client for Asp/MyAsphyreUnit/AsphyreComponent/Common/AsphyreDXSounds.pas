...
uses
  ...
  AsphyreDXDirectX, AsphyreDXClass, AsphyreWave;
...
type
  ...
  TDXSoundNotifyEvent = procedure(Sender: TDXSound; NotifyType: TDXSoundNotifyType) of object;
  ...
  private
    ...
    procedure SetFormHandle(Value: THandle);
  protected
  ...
  public
    ...
    procedure Finalize;
    procedure Initialize(DXSound: TDXSound);
    procedure Restore;
    procedure LoadFromFile(const FileName: string);
    procedure LoadFromStream(Stream: TStream);
    procedure SaveToFile(const FileName: string);
    procedure SaveToStream(Stream: TStream);
    property DXSound: TDXSound read FDXSound;
  ...
  private
    ...
    procedure DXSoundNotifyEvent(Sender: TDXSound; NotifyType: TDXSoundNotifyType);
    procedure SetDXSound(Value: TDXSound);
...
begin
  ...
  begin
    try
      ...
      if Buffer <> nil then FreeAndNil(Buffer);
    end;
  end else
  begin
    Buffer := TDirectSoundBuffer.Create(FDSound);
...
constructor TDXSound.Create(Handle: THandle);
begin
...
procedure TDXSound.RegisterNotifyEvent(NotifyEvent: TDXSoundNotifyEvent);
var
...
procedure TDXSound.UnRegisterNotifyEvent(NotifyEvent: TDXSoundNotifyEvent);
var
  Event: PDXSoundNotifyEvent;
  i: Integer;
begin
...
procedure TDXSound.NotifyEventList(NotifyType: TDXSoundNotifyType);
var
  i: Integer;
begin
...
class function TDXSound.Drivers: TDirectXDrivers;
begin
  Result := EnumDirectSoundDrivers;
end;

procedure TDXSound.DoFinalize;
begin
  if Assigned(FOnFinalize) then FOnFinalize(Self);
end;

procedure TDXSound.DoInitialize;
begin
  if Assigned(FOnInitialize) then FOnInitialize(Self);
end;

procedure TDXSound.DoInitializing;
begin
  if Assigned(FOnInitializing) then FOnInitializing(Self);
end;

procedure TDXSound.DoRestore;
begin
  if Assigned(FOnRestore) then FOnRestore(Self);
end;

procedure TDXSound.Finalize;
begin
...
procedure TDXSound.SetFormHandle;
var
  Level: DWORD;
begin
  FFormHandle := Value;
  if FInitialized then begin
    ...
    FDSound.DXResult := FDSound.ISound.SetCooperativeLevel(FFormHandle, Level);
  end;
end;

procedure TDXSound.Initialize;
const
...
procedure TDXSound.SetDriver(Value: PGUID);
begin
...
procedure TDXSound.SetOptions(Value: TDXSoundOptions);
const
...
procedure TWaveCollection.Initialize(DXSound: TDXSound);
var
  i: Integer;
begin
  Finalize;
...
begin
  ...
  if (Operation=opRemove) and (DXSound=TObject(AComponent)) then
    DXSound := nil;
end;

procedure TCustomDXWaveList.DXSoundNotifyEvent(Sender: TDXSound;
...
procedure TCustomDXWaveList.SetDXSound(Value: TDXSound);
begin
