...
function TDX9Device.InitDevice(): Integer;
var
  Flags: Cardinal;
  bResult : Boolean;
begin
  bResult := (Direct3D = nil) and (Device9 = nil);
...
{$ENDIF}
  ...
  case VertexProcessing of
    vptMixed:
      bResult := Succeeded(Direct3D.CreateDevice(Adapter, D3DDEVTYPE_HAL,
  ...
  begin
    ...
    //if (Result) then
    bResult := Succeeded(Device9.GetDeviceCaps(Caps9));
    Result := 0;
    if (not bResult) then AsphyreError := errRetreiveDeviceCaps;
