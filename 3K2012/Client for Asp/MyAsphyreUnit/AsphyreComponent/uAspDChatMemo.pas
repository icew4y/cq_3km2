...
interface
uses Controls, AspDWinCtl, Types, Windows, Classes, AbstractCanvas, AbstractTextures, AsphyreTypes,
  AsphyreTextureFonts, AspWil, Clipbrd, Graphics, uAspDListView, uAspDControls, Math, Forms;

type

  TAspDChatMemoLines = class(TStringList)
  private
  ...
  TAspDChatMemo = class(TAspDControl) //聊天框专用
  private
    Downed: Boolean;
    FShowScroll: Boolean;
    FScrollSize: Integer;
...
procedure Register;
begin
   RegisterComponents('MirGame', [TAspDChatMemo]);
end;
{ TDChatMemo }

procedure TAspDChatMemo.Add(const S: string; FC, BC: TColor);
var
  TextWidth: Integer;
...
begin
  if ShowScroll then
    TextWidth := GWidth - ScrollSize - TForm(Self.Owner).Canvas.TextWidth('0')
  else TextWidth := GWidth - TForm(Self.Owner).Canvas.TextWidth('0');

  sText := S;

  if TForm(Self.Owner).Canvas.TextWidth(sText) > TextWidth then begin
    Len := Length(sText);
    Temp := '';
    I := 1;
    while True do begin
      ...
      ALine := TForm(Self.Owner).Canvas.TextWidth(Temp);
      if ALine > TextWidth then begin

        ViewItem := TAspDChatMemoLines(FLines).AddItem(Temp, nil);
        ...
        TAspDChatMemoLines(FLines).ItemHeights[FLines.Count - 1] := Max(TAspDChatMemoLines(FLines).ItemHeights[FLines.Count - 1], ALine);
    ...
    if Temp <> '' then begin
      ...
      TAspDChatMemoLines(FLines).ItemHeights[FLines.Count - 1] := Max(TAspDChatMemoLines(FLines).ItemHeights[FLines.Count - 1], TForm(Self.Owner).Canvas.TextWidth(Temp));
    ...
    ViewItem := TAspDChatMemoLines(FLines).AddItem(sText, nil);
    ViewItem.Transparent := False;
    ...
    TAspDChatMemoLines(FLines).ItemHeights[FLines.Count - 1] := Max(TAspDChatMemoLines(FLines).ItemHeights[FLines.Count - 1], TForm(Self.Owner).Canvas.TextWidth(sText));
...
procedure TAspDChatMemo.AddTop(const S: string; FC, BC: TColor; TimeOut: Integer);
var
  TextWidth: Integer;
...
begin
  ...
  if TForm(Self.Owner).Canvas.TextWidth(sText) > TextWidth then begin
    ...
    while True do begin
      ...
      ALine := TForm(Self.Owner).Canvas.TextWidth(Temp);
      if ALine > TextWidth then begin
        ViewItem := TAspDChatMemoLines(FTopLines).AddItem(Temp, nil);
    ...
    ViewItem := TAspDChatMemoLines(FTopLines).AddItem(sText, nil);
    ViewItem.TimeTick := GetTickCount + TimeOut * 1000;
    ViewItem.Transparent := False;
    ViewItem.Caption := sText;
    ViewItem.Color.Up.Color := FC;
...
procedure TAspDChatMemo.BarImageIndexChange(Sender: TObject);
var
...
constructor TAspDChatMemo.Create(AOwner: TComponent);
begin
  inherited Create (AOwner);
  Downed := False;
  FLines := TAspDChatMemoLines.Create;
  FTopLines := TAspDChatMemoLines.Create;
  FAutoScroll := False;
  FShowScroll := True;
  FScrollSize := 16;
  FItemHeight := 12;
...
procedure TAspDChatMemo.Delete(Index: Integer);
begin
  FLines.Delete(Index);
  if FAutoScroll then Previous;
end;

procedure TAspDChatMemo.DeleteTop(Index: Integer);
begin
  FTopLines.Delete(Index);
  if FAutoScroll then Previous;
end;

destructor TAspDChatMemo.Destroy;
begin
  FLines.Free;
  FTopLines.Free;
...
procedure TAspDChatMemo.DirectPaint(dsurface: TAsphyreCanvas);
var
...
begin
  ...
  if Assigned (OnDirectPaint) then
    ...
    for I := 0 to FTopLines.Count - 1 do begin
      ViewItem := TAspDChatMemoLines(FTopLines).Items[I];
...
//            DrawRect(Canvas, PaintRect, vtRect, vbRect, Texture, False);
      ...
      if ViewItem.Caption <> '' then begin
        ...
        if ViewItem.Caption <> '' then begin
          if Font.Bold then begin
            AspTextureFont.BoldTextOut(SurfaceX(GLeft), SurfaceY(GTop), Font.Color, Font.BColor, ViewItem.Caption);
          end else begin
            PaintRect.Left := SurfaceX(GLeft) + nLeft;
            PaintRect.Right := PaintRect.Left + GWidth - ScrollSize;
            PaintRect.Top := SurfaceY(GTop) + nTop + (ItemHeight - TForm(Self.Owner).Canvas.TextHeight('0')) div 2;
            PaintRect.Bottom := PaintRect.Top + 12;
            if not FontBackTransparent then
              //FillRect(Canvas, PaintRect, vtRect, vbRect, Font.BColor);
            dsurface.FillRect(PaintRect, Font.BColor);
            AspTextureFont.TextOut(PaintRect.Left, PaintRect.Top, Font.Color, ViewItem.Caption);
          end;  
        end;
      end;
      Inc(n01);
    ...
    if nMaxValue > 0 then begin
      for I := FTopIndex to FLines.Count - 1 do begin
        ViewItem := TAspDChatMemoLines(FLines).Items[I];
        if WLib <> nil then begin
          FaceIndex := -1;
          if ViewItem.Style = bsButton then begin
            if ViewItem.Down then begin
        ...
        if ViewItem.Caption <> '' then begin
          ...
          if ViewItem.Caption <> '' then begin
            if Font.Bold then begin
              ...
              PaintRect.Left := SurfaceX(GLeft) + nLeft;
              PaintRect.Right := PaintRect.Left + TForm(Self.Owner).Canvas.TextWidth(ViewItem.Caption)+3;
              PaintRect.Top := SurfaceY(GTop) + nTop + (ItemHeight - TForm(Self.Owner).Canvas.TextHeight('0')) div 2;
              PaintRect.Bottom := PaintRect.Top + 12;
              if not FontBackTransparent then
                //FillRect(Canvas, PaintRect, vtRect, vbRect, Font.BColor);
...
procedure TAspDChatMemo.DoResize;
var
...
begin
  if FShowScroll then begin
    ...
    nMaxScrollValue := GHeight - FPrevImageSize - FNextImageSize - FBarImageSize;
                            // TasNat 防止除0错误 2012-03-03 17:25:50
    if (nMaxValue > 0) and (nMaxValue <> VisibleHeight) and (nMaxScrollValue > 0) then
...
procedure TAspDChatMemo.DoScroll(Value: Integer);
begin
  FTopIndex := (Position - Value) div ItemHeight;
end;

procedure TAspDChatMemo.First;
var
  P: Integer;
  nMaxScrollValue: Integer;
  nMaxValue: Integer;
begin
  ...
  if FShowScroll then begin
    ...
    if (nMaxValue > 0) and (nMaxScrollValue > 0) and (nMaxValue > VisibleHeight) then
      FBarTop := Max(Round(FPosition * nMaxScrollValue / (nMaxValue - VisibleHeight)), 0)
    else
      FBarTop := 0;
...
function TAspDChatMemo.GetVisibleHeight: Integer;
begin
  Result := GHeight - OffSetY;
end;

function TAspDChatMemo.InBarRange(X, Y: Integer): Boolean;
var
  nHeight: Integer;
  nMaxValue: Integer;
  nBarTop: Integer;
...
function TAspDChatMemo.InNextRange(X, Y: Integer): Boolean;
begin
  Result := (X >= GLeft + GWidth - FScrollSize) and (Y >= GTop + (GHeight - FNextImageSize));
end;

function TAspDChatMemo.InPrevRange(X, Y: Integer): Boolean;
begin
  Result := (X >= GLeft + GWidth - FScrollSize) and (Y <= GTop + FPrevImageSize);
end;

procedure TAspDChatMemo.Insert(Index: Integer; const S: string; FC, BC: TColor);
var
  TextWidth: Integer;
...
begin
  ...
  if TForm(Self.Owner).Canvas.TextWidth(sText) > TextWidth then begin
    ...
    while True do begin
      ...
      ALine := TForm(Self.Owner).Canvas.TextWidth(Temp);
      if ALine > TextWidth then begin
        TAspDChatMemoLines(FLines).Insert(Index, Temp);
...
procedure TAspDChatMemo.InsertTop(Index: Integer; const S: string; FC, BC: TColor;
  TimeOut: Integer);
var
  TextWidth: Integer;
...
begin
  ...
  if TForm(Self.Owner).Canvas.TextWidth(sText) > TextWidth then begin
    ...
    while True do begin
      ...
      ALine := TForm(Self.Owner).Canvas.TextWidth(Temp);
      if ALine > TextWidth then begin
        TAspDChatMemoLines(FTopLines).Insert(Index, Temp);
...
function TAspDChatMemo.KeyDown(var Key: Word; Shift: TShiftState): Boolean;
begin
  Result := False;
...
procedure TAspDChatMemo.LoadFromFile(const FileName: string);
begin
  FLines.LoadFromFile(FileName);
  FTopLines.Clear;
end;

function TAspDChatMemo.MaxValue: Integer;
begin
...
function TAspDChatMemo.MouseDown(Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer): Boolean;
...
function TAspDChatMemo.MouseMove(Shift: TShiftState; X, Y: Integer): Boolean;
var
  P: Integer;
  nHeight: Integer;
  nMaxValue: Integer;
...
function TAspDChatMemo.MouseUp(Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer): Boolean;
begin
  Result := FALSE;
...
procedure TAspDChatMemo.NextImageIndexChange(Sender: TObject);
var
  d: TAsphyreLockableTexture;
  nIndex: Integer;
begin
...
procedure TAspDChatMemo.PrevImageIndexChange(Sender: TObject);
var
  d: TAsphyreLockableTexture;
  nIndex: Integer;
begin
...
procedure TAspDChatMemo.Previous;
var
  P: Integer;
  nMaxScrollValue: Integer;
  nMaxValue: Integer;
...
procedure TAspDChatMemo.ScrollImageIndexChange(Sender: TObject);
var
  d: TAsphyreLockableTexture;
  nIndex: Integer;
begin
...
procedure TAspDChatMemo.SetExpandSize(Value: Integer);
var
...
procedure TAspDChatMemo.SetItemHeight(Value: Integer);
begin
  FItemHeight := Max(Value, 1);
end;

procedure TAspDChatMemo.SetItemIndex(Value: Integer);
var
  nItemCount: Integer;
begin
...
procedure TAspDChatMemo.SetLines(Value: TStrings);
begin
  FLines.Clear;
  FLines.AddStrings(Value);
end;

procedure TAspDChatMemo.SetPosition(Value: Integer); //设置滚动指针
var
...
begin
  ...
  if FShowScroll then begin
    ...
    if (nMaxValue > 0) and (nMaxScrollValue > 0) and
    (nMaxValue > VisibleHeight) //修复除0错误By TasNat at: 2012-03-07 16:01:45
    then
      FBarTop := Max(Round(FPosition * nMaxScrollValue / (nMaxValue - VisibleHeight)), 0)
    else
      FBarTop := 0;
...
procedure TAspDChatMemo.SetScrollSize(Value: Integer);
begin
...
procedure TAspDChatMemo.SetTopLines(Value: TStrings);
begin
  FTopLines.Clear;
  FTopLines.AddStrings(Value);
end;
procedure TAspDChatMemo.SetVisibleItemCount(Value: Integer);
var
  nMaxValue, nMaxScrollValue: Integer;
begin
...
function TAspDChatMemoLines.AddItem(const S: string; AObject: TObject): pTViewItem;
begin
  AddObject(S, AObject);
  Result := @FItemList[Length(FItemList) - 1];
end;

function TAspDChatMemoLines.AddObject(const S: string; AObject: TObject): Integer;
var
  ViewItem: pTViewItem;
begin
...
procedure TAspDChatMemoLines.Clear;
var
  I: Integer;
begin
...
constructor TAspDChatMemoLines.Create;
begin
  inherited;
  FItemList := nil;
  FItemHeightList := nil;
...
procedure TAspDChatMemoLines.Delete(Index: Integer);
var
  I: Integer;
begin
...
destructor TAspDChatMemoLines.Destroy;
var
  I: Integer;
begin
  for I := 0 to Length(FItemList) - 1 do begin
...
function TAspDChatMemoLines.GetItem(Index: Integer): pTViewItem;
begin
  Result := @FItemList[Index];
end;

function TAspDChatMemoLines.GetItemHeight(Index: Integer): Integer;
begin
  Result := FItemHeightList[Index];
end;

procedure TAspDChatMemoLines.InsertObject(Index: Integer; const S: string;
  AObject: TObject);
var
  I: Integer;
  ViewItem: pTViewItem;
...
procedure TAspDChatMemoLines.SetItemHeight(Index, Value: Integer);
begin
