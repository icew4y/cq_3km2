...
interface
uses Classes, AspDWinCtl, uAspDControls, Controls, AbstractCanvas, AbstractTextures,
     AsphyreTextureFonts, uAspDScrollBar, Types, Graphics, AspWil;
...
type
  ...
  public
    ...
    property Owner: TAspDTreeView read FOwner write FOwner;
  ...
  TAspDTreeView = class(TAspDControl)
  private
    FHScrollBar: TAspDScrollBar;
    FList: TList;
    FTreeNodeList: TList;
    ...
    function GetCount: Integer;
    procedure SetHScrollBar(Value: TAspDScrollBar);
  protected
  public
  ...
  published
    ...
    property HScrollBar: TAspDScrollBar read FHScrollBar write SetHScrollBar;
...
procedure Register;
begin
   RegisterComponents('AspMirGame', [TAspDTreeView]);
end;
{ TDTreeNode }
...
procedure TAspDTreeView.Add(Item: TDTreeNode);
begin
  FList.Add(Item);
  Item.Owner := Self;
  Item.Parent := nil;
...
function TAspDTreeView.Click(X, Y: integer): Boolean;
var
  I: Integer;
  nWidth: Integer;
  nLeft: Integer;
...
begin
  ...
  if inherited Click(X, Y) then begin
    ...
    if TreeNode <> nil then begin
      nLeft := (TreeNode.Level + 1) * AspTextureFont.TextWidth('0')*2;//MainForm.Canvas.TextWidth('0') * 2;
...
constructor TAspDTreeView.Create(AOwner: TComponent);
begin
...
procedure TAspDTreeView.Delete(Item: TDTreeNode);
begin
  FList.Remove(Item);
end;

destructor TAspDTreeView.Destroy;
var
  I: Integer;
  TreeNode: TDTreeNode;
begin
...
procedure TAspDTreeView.DirectPaint(dsurface: TAsphyreCanvas);
var
    ...
    OnDirectPaint (self, dsurface)
      ...
      for I := nIndex to {nCount}TreeNodeList.Count - 1 do begin
        ...
        if TreeNode.Expand then begin //¡ú
          nHeight := AspTextureFont.TextHeight('0');//MainForm.Canvas.TextHeight('0');
          nWidth := {MainForm.Canvas.TextWidth('0')}AspTextureFont.TextWidth('0') - 3;
          X1 := TreeNode.Level * ({MainForm.Canvas.TextWidth('0')}AspTextureFont.TextWidth('0')) * 2; //TreeNode.Level * nWidth;
          ...
          nWidth := AspTextureFont.TextWidth('0') + 1;
          nHeight := AspTextureFont.TextWidth('0') - 4;

          X1 := TreeNode.Level * (AspTextureFont.TextWidth('0')) * 2; //TreeNode.Level * nWidth;
          Y1 := nTop + (FHScrollBar.ItemHeight - nHeight) div 2;
          X2 := X1 + nWidth div 2;
        ...
        nLeft := (TreeNode.Level + 1) * (AspTextureFont.TextWidth('0')) * 2;
        if TreeNode.Caption <> '' then begin
          OldSize := FontManager.FontSize;
          OldFontName := FontManager.FontName;
          ...
          if Font.Name = '' then Font.Name := 'ËÎÌå';
          FontManager.SetFont(Font.Name, Font.Size);


          if Font.Bold then
            AspTextureFont.BoldTextOut(SurfaceX(GLeft+nLeft), SurfaceY(GTop+nTop) + (FHScrollBar.ItemHeight - AspTextureFont.TextHeight('0')) div 2, Font.Color, Font.BColor, TreeNode.Caption, Font.Style)
          else
            AspTextureFont.TextOut(SurfaceX(GLeft+nLeft), SurfaceY(GTop+nTop) + (FHScrollBar.ItemHeight - AspTextureFont.TextHeight('0')) div 2, Font.Color, TreeNode.Caption, Font.Style);

          FontManager.SetFont(OldFontName, OldSize);
        end;
        Inc(n01);
...
function TAspDTreeView.Get(Index: Integer): TDTreeNode;
begin
...
function TAspDTreeView.GetCount: Integer;
begin
  Result := FList.Count;
end;

function TAspDTreeView.GetItem(Index: Integer): TDTreeNode;
begin
  if (Index >= 0) and (Index < FList.Count) then
    Result := FList.Items[Index]
  else
...
function TAspDTreeView.IndexOf(Item: TDTreeNode): Integer;
var
  D, P: TDTreeNode;
begin
  Result := -1;
...
function TAspDTreeView.MouseDown(Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer): Boolean;
var
  I: Integer;
  nLeft: Integer;
...
function TAspDTreeView.MouseMove(Shift: TShiftState; X, Y: Integer): Boolean;
var
  I: Integer;
  nLeft: Integer;
  nRow: Integer;
...
function TAspDTreeView.MouseUp(Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer): Boolean;
var
  I: Integer;
  nLeft: Integer;
...
procedure TAspDTreeView.RefTreeNodeList;
var
  I: Integer;
  TreeNode: TDTreeNode;
begin
...
procedure TAspDTreeView.SetHScrollBar(Value: TAspDScrollBar);
begin
  FHScrollBar := Value;
end;
