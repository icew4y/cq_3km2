...
uses
  Windows, Forms, SysUtils, Classes, Grobal2, ExtCtrls, HUtil32, AsphyreDXSounds;
...
type
  ...
  private
    ...
    procedure PlayPoHun(Sender:TObject);
    procedure PlayPiXing(Sender:TObject);
  public
    constructor Create;
    destructor Destroy; override;
    
    procedure Play(Typ:Integer);
  end;
...
const
  ...
  {$IFEND}
   ...
   BoxexChange_ground    = 'wav\box2exchange.wav'; //3¸ñÌî³äÊ±ºòµÄÉùÒô
   BoxonCeagain_ground   = 'wav\box2onceagain.wav'; //12¸öÎïÆ·È«²¿¸üÐÂÊ±ºòµÄÉùÒô
   ...
   Lava_ground           ='wav\S1-1.wav'; //ÑÒ½¬ÉùÒô
   dnsbaozhu             ='wav\dnsbaozhu.wav'; //¸»¹óÊÞ±¬ÖñÉùÒô
   cboZs1_start_m        ='wav\cboZs1_start_m.wav'; //Á¬»÷ÄÐÕ½Ê¿Èý¾øÉ±ÉùÒô
   cboZs1_start_w        ='wav\cboZs1_start_w.wav'; //Á¬»÷Å®Õ½Ê¿Èý¾øÉ±ÉùÒô
   cboZs2_start          ='wav\cboZs2_start.wav'; //Á¬»÷Õ½Ê¿×·ÐÄ´ÌÉùÒô
   cboZs3_start          ='wav\cboZs3_start.wav'; //Á¬»÷Õ½Ê¿¶ÏÔÀÕ¶ÏÂµ¶Ê±ÉùÒô
   cboZs3_start_m        ='wav\cboZs3_start_m.wav'; //Á¬»÷ÄÐÕ½Ê¿¶ÏÔÀÕ¶ÉùÒô
   cboZs3_start_w        ='wav\cboZs3_start_w.wav'; //Á¬»÷Å®Õ½Ê¿¶ÏÔÀÕ¶ÉùÒô
   cboZs4_start          ='wav\cboZs4_start.wav'; //Á¬»÷Õ½Ê¿ºáÉ¨Ç§¾üÉùÒô
   cboZs7_start          ='wav\cboZs7_start.wav'; //Á¬»÷Õ½Ê¿¶ÏÔÀÕ¶ÆðÊÖÉùÒô
   byjm_injured_ground   ='wav\byjm_injured.wav'; //Õ½½«ËÀÍöÉùÒô
   cqsbz_tsgj_ground     ='wav\cqsbz_tsgj.wav';//Ìì½«ÆÕÍ¨¹¥»÷
   sbz_injured_ground    ='wav\sbz_injured.wav';//Õ½½«ÊÜ¹¥»÷ÉùÒô
   sbz_tsgj_ground       ='wav\sbz_tsgj.wav'; //Ñ©ÓòÕ½½«¹¥»÷ÉùÒô
   cqsbz_injured_ground  ='wav\cqsbz_injured.wav'; //Ñ©ÓòÊÌÎÀÊÜ¹¥»÷ÉùÒô
   xsls_injured_ground   ='wav\xsls_injured.wav'; //Ñ©ÓòÌì½«ÊÜ¹¥»÷ÉùÒô
   xsls_death_ground     ='wav\xsls_death.wav'; //Ñ©ÓòÌì½«ËÀÍöÉùÒô
   x60182_ground         ='wav\60182.wav'; //Ñ©ÓòÄ§Íõ¹¥»÷ÉùÒô
   bq_injured_ground     ='wav\bq_injured.wav'; //Ñ©ÓòÄ§ÍõÊÜ¹¥»÷ÉùÒô
   bq_death_ground       ='wav\bq_death.wav'; //Ñ©ÓòÄ§ÍõËÀÍöÉùÒô
   xsws_pbec_ground      ='wav\xsws_pbec.wav'; //Ñ©ÓòÎÀÊ¿³öÀ´ÉùÒô
   xsws_tsgj_ground      ='wav\xsws_tsgj.wav'; //Ñ©ÓòÎÀÊ¿ÆÕÍ¨¹¥»÷ÉùÒô
   xsws_injured_ground   ='wav\xsws_injured.wav'; //Ñ©ÓòÎÀÊ¿ÊÜ¹¥»÷ÉùÒô
   xsws_death_ground     ='wav\xsws_death.wav'; //Ñ©ÓòÎÀÊ¿ËÀÍöÉùÒô
...
procedure TSoundPlayer.PlayPoHun(Sender:TObject);
var Timer:TTimer;
begin
  Timer:=TTimer(Sender);
  PlaySound(57);
...
procedure TSoundPlayer.PlayPiXing(Sender:TObject);
var Timer:TTimer;
begin
  Timer:=TTimer(Sender);
  PlaySound(124);
...
procedure TSoundPlayer.Play(Typ:Integer);
var Timer:TTimer;
begin
...
procedure MyPlaySound (wavname: string);
begin
  if FileExists (g_ParamDir+wavname) and g_boSound then begin
    try
