...
uses
  Windows, Classes, AbstractCanvas, AbstractTextures, Grobal2, MShare;
const
...
begin
   if m_Dsurface <> nil then
      if m_boBlend then backsurface.DrawBlend(ax + m_nPx, ay + m_nPy, m_Dsurface)//DrawBlend (backsurface, ax + m_nPx, ay + m_nPy, m_Dsurface, 1, 120)
...
begin
  ...
  case m_nEventType of
    ...
    ET_NOTGOTO: begin
      m_Dsurface := g_WMagic10Images.GetCachedImage(Mag113_NOTGOTOBase+(m_dwCurframe mod 15), m_nPx, m_nPy);
      m_boBlend := TRUE;
      m_nLight := 1;
    end;
    ET_FIRE: begin
