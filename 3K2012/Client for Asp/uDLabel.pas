...
interface
uses Classes, DWinCtl, Graphics, AbstractCanvas, AbstractTextures, AsphyreTextureFonts,
     Controls, Windows, Math;
...
type
  ...
  published
    ...
    property Down: Boolean read FDown write FDown; //ÊÇ·ñÔÊÐí°´ÏÂÊÂ¼þ
...
begin
  ...
  AspTextureFonts.SetFont('ËÎÌå', Font.Size);
  if Caption <> '' then begin
    if Caption = '-' then begin
      ...
      if FBold then begin
        if FDown and Downed then begin
        
        end else begin
          AspTextureFont.BoldTextOut(nX+1,
            SurfaceY(GTop)+1 + (GHeight - AspTextureFont.TextHeight('0')) div 2, FCurrColor, FBoldColor, Caption, Font.Style);
        end;
      end else begin
        if FDown and Downed then begin
  ...
  AspTextureFonts.SetFont('ËÎÌå', OldSize);
...
begin
  ...
  if Downed and FDown then begin
...
//    if MouseDownControl = Self then FBorderCurrColor := FDownBorder;  ×¢ÊÍ
