...
uses
  ...
  DXDraws, Grobal2, HUtil32, SDK, Share, Forms, Wil;
type
   TextOutImage = record
...
var
   DropItems: TList;  //lsit of TClientEffecItem
   //{$IF Version <> 1}
   //字用图输出 20080916
   //TextOutList: TGList;
...
procedure ClearBag;
function  AddItemBag (cu: TClientItem): Boolean; overload;
function AddItemBagLock(cu: TClientItem): Boolean;
function DelItemBagLock(cu: TClientItem): Boolean;
function  AddHeroItemBag (cu: TClientItem): Boolean; overload;//英雄包 2007.10.23
procedure ArrangeHeroItemBag;//英雄包 2007.10.23
...
function  GetMagicIcon (Effect, Level: Byte; Id: Word; LevelEx: Byte; var Icon: Integer): TWMImages;
...
function GetItemEffectWil(idx: Byte): TWMImages;
implementation
      ...
      if g_ItemArr[i].Item.S.Name <> '' then begin
         ...
         {for k:=0 to 9 do begin
            if (ItemArr[i].S.Name = DealItems[k].S.Name) and (ItemArr[i].MakeIndex = DealItems[k].MakeIndex) then begin
               FillChar (ItemArr[i], sizeof(TClientEffecItem), #0);
               //FillChar (DealItems[k], sizeof(TClientEffecItem), #0);
            end;
         end; }
         //若有移动的物品
...
begin
   ...
   case smode of //StdMode
      ...
      {$ELSE}//增加1.76四格支持By TasNat at: 2012-10-20 12:05:54
      25: if g_boShowNewItem then Result := U_BUJUK else Result := U_ARMRINGL;
      {$IFEND}
...
begin
  Result := '';
  if g_TipsList.Count > 0 then begin
    Randomize(); //随机种子
...
begin
 ...
 { New(PixelsOutImage);
  ...
  PixelsOutImage.Surface.SystemMemory := TRUE;
  PixelsOutImage.Surface.SetSize (Size, Size);
  PixelsOutImage.Surface.Canvas.Brush.Color := Color;
  PixelsOutImage.Surface.Canvas.FillRect(PixelsOutImage.Surface.ClientRect);
  PixelsOutImage.Surface.Canvas.Release;
  surface.Draw (x, y, PixelsOutImage.Surface.ClientRect, PixelsOutImage.Surface, False);
  PixelsOutImage.FColor := Color;
  PixelsOutImage.BColor := Size;
  PixelsOutImage.dwLatestTime := GetTickCount;
