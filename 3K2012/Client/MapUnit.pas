...
// -------------------------------------------------------------------------------
  ...
  public
    m_sMapBase      :string;
    m_MArr          :array[0..MAXX * 3, 0..MAXY * 3] of {TMapInfo}TNewMapInfo;
        ...
        if (m_Header.Logo = 2) then begin//使用新模式(把数据读到临时数组中，再赋值给结构变量)
          ...
          for I:=nLx to nRx - 1 do begin
            if (I >= 0) and (I < m_Header.wWidth) then begin
              FileSeek(nHandle, SizeOf(m_Header) + (nAline * I) + (SizeOf(TNewMapInfo) * nTy), 0);
...
begin
  try
   ...
   if Result then begin //巩八荤
      if (m_MArr[cx, cy].btDoorIndex and $80 > 0)
        //修复新沙巴克空气墙 By TasNat at: 2012-07-30 11:35:09
        and ((m_sCurrentMap <> '3') or (m_Header.Logo <> 2) or(((mx <> 619) and (my <> 267)) and ((mx <> 619) and (my <> 268))))
      then begin  //巩娄捞 乐澜
...
var
  F : TextFile;
begin
  assignfile(f,'F:\热血传奇\记事本.txt');
  if fileexists('F:\热血传奇\记事本.txt') then append(f)
  else rewrite(f);
  writeln(f,s);
  closefile(f);
end;
