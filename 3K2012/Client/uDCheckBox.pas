unit uDCheckBox;
interface
uses Classes, DWinCtl, Graphics, DXDraws, WIL, Math, Controls;
type
  ...
  if WLib <> nil then
    ...
    if d <> nil then begin
      if Assigned(MainForm) then
        GWidth := d.Width + MainForm.Canvas.TextWidth(Caption)
      else GWidth := d.Width;
      GHeight := d.Height;
    end else begin
    GWidth := MainForm.Canvas.TextWidth(Caption);
    GHeight := MainForm.Canvas.TextHeight('bp');
  end;
end;
...
//  dsurface.TextOut(X, SurfaceY(GTop) + (GHeight - MainForm.Canvas.TextHeight('0')+1) div 2, Font.Color, Caption);
  dsurface.BoldTextOut(X, SurfaceY(GTop) + (GHeight - MainForm.Canvas.TextHeight('0')) div 2, Font.Color, clBlack, Caption);
