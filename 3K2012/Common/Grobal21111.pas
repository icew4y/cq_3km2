...
const
  ...
  M2Version = 1; //0-0627版程序(非连击+内功) 1-最新版(内功+连击版) 2-1.76版(无内功,无连击)
  {$IF M2Version = 2}
  HEROVERSION = 0; //0-企业版(无英雄) 1-英雄版
  {$ELSE}
  HEROVERSION = 1; //0-企业版(无英雄) 1-英雄版
  {$IFEND}
  CLIENT_VERSION_NUMBER = 920111117;//9+客户端版本号 20091025，当小于20080512时，Run网关会把玩家T了，RUN代码中检查自己的G单元的日期(20080512)
  MAXPATHLEN = 255;                               
  DIRPATHLEN = 80;
  MAPNAMELEN = 16;//地图名长度
...
//----------------------------------------------------------------------------
  ...
  SM_SENDLOGINRANDOMID = 20179; //发送网关随机标实
...
//--------------------------------连击相关-------------------------------------
  ...
  SM_LOGINGATECHECK_OK = 20241; //网关验证过
...
type
  ...
  TAddAbility = record //OK    //Size 40
    ...
    wUnParalysisRate: Byte;//防麻机率(辉煌衣服) 20100513
  end;
...
{$IFEND}  
 ...
 {$IF M2Version <> 2}
  ...
  wClientHumTitles = packed record//发送客户端称号结构
...
//------------------------------------------------------------------------------
  TBoxsInfo = record //宝箱数据结构 20090225 修改
    ...
    nItemRace: Word;//几率
    StdItem: {TClientItem}TClientEffecItem;//20110919
  end;
...
//------------------------------------------------------------------------------
  ...
  TClientDealOffInfo = packed record //客户端元宝寄售数据结构  20080317
    ...
    dSellDateTime: TDateTime;//寄售时间
    nSellGold: Integer;//交易的元宝数
    UseItems: array[0..9] of {TClientItem}TClientEffecItem;//物品 //20110919
    N: Byte;//交易识标
  end;
...
//------------------------------------------------------------------------------
  ...
  TRefineDrumItemInfo = packed record // Size 36 淬炼军鼓(同时可以淬炼其他物品)数据结构  20080502
    ItemName0:string; // 合成的物品名称
    ItemName1: string;// 主物品名称
    ItemName2:array[0..5] of String;  // 辅助物品名称
    ItemName2Count:Integer;
    
    ItemName3:array[0..5] of String;  // 需要元宝，灵狐等等
    ItemName3Count:Integer;
    UseItemName3Num:array[0..5] of Integer;  // 需要的数量
...
//------------------------------------------------------------------------------
  ...
  TUseItemNames = array[0..13] of string[16];
...
//------------------------------------------------------------------------------
  ...
  TUserStateInfo = record
    ...
    UseItems: array[0..13] of TClientEffecItem;//20110912 加入物品特效
    btJob: Byte;//职业 20110113
    {$IF M2Version <> 2}
    wHumTitles: wClientHumTitles;//称号
...
{$IFEND}  
 ...
 { pTPowerBlock = ^TPowerBlock;      //20110222 注释
  ...
  TShopItem = packed record//个人商店物品
    Item: {TClientItem}TClientEffecItem;//20110919
    nPic: Integer;//价格
    boCls: Boolean;//T-元宝 F-金币
  end;
