unit AsphyreFonts;
//---------------------------------------------------------------------------
// AsphyreFonts.pas                                     Modified: 19-May-2008
// Asphyre Bitmap fonts with Unicode support                     Version 2.01
//---------------------------------------------------------------------------
// Important Notice:
//
...
//---------------------------------------------------------------------------
 ...
 private
  ...
  function IsTextChar(Ch: WideChar): Boolean;
...
{$IFDEF VER185}
 State.Colors[0]:= CardinalToAspColor(Colors[0]);
 State.Colors[1]:= CardinalToAspColor(Colors[1]);
{$ELSE}
...
begin
 ...
 if (TagName = '/') then
  begin
   if (StyleStack.Count > 1) then PopStyle();
   Exit;
  end;
...
{$IFDEF VER185}
   Colors[0]:= AspColorToCardinal(cLerp(Style.Colors[0], Style.Colors[1], Entry.Top / FFontSize.y));
   Colors[1]:= AspColorToCardinal(cLerp(Style.Colors[0], Style.Colors[1],
...
{$ENDIF}
   ...
   // -> Display the letter.
   Canvas.UseImage(Image, pxBounds4f(Entry.Pos.x, Entry.Pos.y,
...
{$IFDEF VER185}
 TextOut(Point2pxToPoint2(DrawPos), Text, Colors, Alpha);
{$ELSE}
...
//---------------------------------------------------------------------------
function TAsphyreFont.IsTextChar(Ch: WideChar): Boolean;
var
 IsDiv, IsPara: Boolean;
begin
 IsDiv := Pos(Ch, FDivSet) <> 0;
 IsPara:= Pos(Ch, FParaSet) <> 0;

 Result:= (IsDiv)or(IsPara)or(Ch = #32);
end;
...
begin
 ...
 while (Step <= Length(Text))and(IsTextChar(Text[Step])) do Inc(Step);
