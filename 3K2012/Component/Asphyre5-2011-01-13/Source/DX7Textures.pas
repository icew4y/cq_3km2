unit DX7Textures;
//---------------------------------------------------------------------------
// DX7Textures.pas                                      Modified: 19-May-2008
...
//---------------------------------------------------------------------------
uses
  DirectDraw7, Direct3D7, Types, Vectors2px, AsphyreTypes, AbstractTextures,
  SystemSurfaces;
...
type
  ...
  private
    ...
    function GetLockRectPtr(LockedRect: PRect): PRect;
...
//---------------------------------------------------------------------------
uses
  Windows, AsphyreUtils, AsphyreErrors, AsphyreConv, DX7Types;
...
function TDX7LockableTexture.GetLockRectPtr(LockedRect: PRect): PRect;
begin
  Result := LockedRect;

  if (LockedRect.Left = 0) and
    (LockedRect.Top = 0) and
    (LockedRect.Right = Width) and
    (LockedRect.Bottom = Height) then Result := nil;
end;
...
{$IFDEF VER185}
  if Vectors2pxEqual(Size, ZeroPoint2px) then
{$ELSE}
...
{$ENDIF}
  ...
  begin
    ...
    begin
      Pixel32toXArray(Buffer.Scanline[Index], LinePtr, FFormat, Width);
    end;
  end;
