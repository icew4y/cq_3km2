...
//---------------------------------------------------------------------------
uses
  Windows, Direct3D9, DX9Types, D3DX9, Types, SysUtils, AsphyreTypes, AbstractTextures;
...
function TDX9LockableTexture.LoadFromFile(const AFileName: string; AMipMapping: Boolean = True): Boolean;
begin
  Result := False;
  if Active then DestroyTexture;
  if (not Active) then
    Result := Succeeded(D3DXCreateTextureFromFile(Device9, PChar(AFileName), FTexture));
  if not Result then
    Result := inherited LoadFromFile(AFileName, AMipMapping);
end;
//---------------------------------------------------------------------------

function TDX9LockableTexture.SaveToFile(const AFileName: string): Boolean;
begin
...
function TDX9LockableTexture.CreateTextureInstance(SrcData: Pointer; SrcDataSize: LongWord; BitCount: Byte): Boolean;
var
  Levels: Integer;
  DFormat: TD3DFormat;
...
begin
  ...
  Result := Succeeded(D3DXCreateTextureFromFileInMemoryEx(Device9, SrcData, SrcDataSize,
...
function TDX9LockableTexture.CreateTexture(SrcData: Pointer; SrcDataSize: LongWord; BitCount: Byte): Boolean;
begin
  ComputeParams();
