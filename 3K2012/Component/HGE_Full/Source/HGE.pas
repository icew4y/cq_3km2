...
** Continuing on from Erik's excellent conversion...
** Delphi DirectX9 conversion by Rimmer@2007
*)

// DirectX9 is default Interface and requires no supporting Dll's
// Set Define flag below to use previous DirectX8 Interface which requires D3DX81ab.dll
...
type
  ...
  ['{9D5C8783-956C-42E1-9307-CAD03DEC1E7A}']
    ...
    procedure SetPixel(const x, y: Integer; const Value: Cardinal);
    ...
    {$ENDIF}
    property Pixels[const x, y: Integer]: Cardinal read GetPixel write SetPixel;
...
type
  ...
  THGECallBuildEvent = procedure (EventType, Key, Scan, Flags, X, Y: Integer);
...
type
  ...
  ['{14AD0876-19A5-4B13-B2D8-46ECE1E336BA}']
    ...
    procedure System_SetState(const State: THGEFuncState; const Value: THGECallBuildEvent); overload;
...
uses
  ...
  {$IFDEF HGE_DX8} D3DX81mo {$ELSE} D3DX9 {$ENDIF}, ZipUtils, UnZip;
...
type
  ...
  private
    ...
    FProcBuildFunc: THGECallBuildEvent;
    FIcon: String;
    FWinTitle: String;
...
type
  ...
  public
    {$IFDEF HGE_DX8}
    constructor Create(const AHandle: IDirect3DTexture8; const AOriginalWidth, AOriginalHeight: Integer);
    {$ELSE}
...
type
  ...
  public
    {$IFDEF HGE_DX8}
    constructor Create(const AWidth, AHeight: Integer; const ATex: ITexture; const ADepth: IDirect3DSurface8);
    {$ELSE}
...
{ TTexture }
{$IFDEF HGE_DX8}
constructor TTexture.Create(const AHandle: IDirect3DTexture8; const AOriginalWidth, AOriginalHeight: Integer);
{$ELSE}
...
begin
  Result := Bounds(0, 0, FPatternWidth, FPatternHeight);
end;
...
function TTexture.GetPixel(const x, y: Integer): Cardinal;
var
  Bits: PLongWord;
begin
  Result := 0;
  if (x < 0) or (y < 0) or (x >= FOriginalWidth) or (y >= FOriginalHeight) then Exit;

  Bits := Lock(True, x, Y, FOriginalWidth, y);
  try
...
procedure TTexture.SetPatternHeight(const Value: Integer);
begin
  FPatternHeight := Value;
end;

procedure TTexture.SetPatternWidth(const Value: Integer);
begin
  FPatternWidth := Value;
end;

procedure TTexture.SetPixel(const x, y: Integer; const Value: Cardinal);
var
  Bits: PLongWord;
begin
  if (x < 0) or (y < 0) or (x >= FOriginalWidth) or (y >= FOriginalHeight) then Exit;

  Bits := Lock(False, x, Y, FOriginalWidth, y);
  try
    if (Bits = nil) then Exit;
    Bits^ := Value;
  finally
...
{ TTarget }
{$IFDEF HGE_DX8}
constructor TTarget.Create(const AWidth, AHeight: Integer; const ATex: ITexture; const ADepth: IDirect3DSurface8);
{$ELSE}
...
begin
  ...
  end else if (EventType = HGE.INPUT_KEYUP) then begin
  ...
  if (EventType = INPUT_MBUTTONDOWN) then begin
    ...
    SetCapture(FWnd);
    FCaptured := True;
  end else if (EventType = INPUT_MBUTTONUP) then begin
    //FKeyz[Key] := FKeyz[Key] or 2;
    ReleaseCapture;
  ...
  if Assigned(FProcBuildFunc) then
    FProcBuildFunc(EventType, Key, Scan, Flags, X, Y);
 {
  ...
  if (FQueue = nil) then
    ...
    Last := FQueue;
    while Assigned(Last.Next) do begin
      Last := Last.Next;
    end;
    Last.Next := EPtr;   
  end;
    ...
    if EventType = 4 then Form1.Memo1.Lines.Add('1');
...
// Set up Windowed presentation parameters
  ...
  if (FHGEFPS = HGEFPS_VSYNC) then begin
    {Note: D3DSWAPEFFECT_COPY_VSYNC not supported in DirectX9 or is it ????}
    {$IFDEF HGE_DX8}
...
begin
 {Note: Why is this commented ????}
//if(FD3DDevice.TestCooperativeLevel <> D3DERR_DEVICELOST) then Exit;
...
begin
  ...
  if (Target <> FCurTarget) then begin
    ...
    {$IFDEF HGE_DX8}
    if (Failed(FD3DDevice.SetRenderTarget(Surf,Depth)))
    {$ELSE}
    if (Failed(FD3DDevice.SetRenderTarget(0,Surf)))
    {$ENDIF}
    then begin
...
begin
  ...
  if (not (Filename[2] in ['\','/',':'])) then begin
    // Load from pack
...
procedure THGEImpl.System_SetState(const State: THGEFuncState;
  const Value: THGECallBuildEvent);
begin
  FProcBuildFunc := Value;
end;
