...
(*
1、修改 System_Initiate
2、增加 INPUT_KEYPRESS
3、增加 WM_CHAR WM_IME_CHAR
4、System_Start过程增加：(不增加则WM_CHAR 无作用)
...
(*
** HGE Handle types
2011-03-05 增加Pixels获得点像素
2011-03-06 增加ClientRect 图像矩形
*)
type      
  ITexture = Interface
  ['{9D5C8783-956C-42E1-9307-CAD03DEC1E7A}']
    ...
    function Lock(const ReadOnly: Boolean = True; const Left: Integer = 0;
      ...
      const Height: Integer = 0): Pointer;
    procedure Unlock;
...
var
  SystemLogMsgList: TGStringList;

procedure MainOutMessage(const sMsg: string; wLevel: Word = 0);
...
begin
  case Msg of
    ...
    WM_CHAR:
      begin
        PHGE.BuildEvent(INPUT_KEYPRESS, WParam, 0, 0, LoWordInt(LParam), HiWordInt(LParam));
        Result := 0;
        Exit;
      end;
    WM_IME_CHAR:
...
procedure MainOutMessage(const sMsg: string; wLevel: Word);
begin
  SystemLogMsgList.Lock;
  try
    SystemLogMsgList.Add('[' + DateTimeToStr(Now()) + '] ' + sMsg);
  finally
    SystemLogMsgList.UnLock;
  end;
end;
...
begin
  if EventType = 4 then
  MainOutMessage(IntToStr(EventType));
  New(EPtr);
...
begin
  ...
  if (not GfxInit) then begin
    //System_Shutdown; 如果执行则 工程里messagebox 不起作用
    Exit;
  end;
...
begin
  SystemLogMsgList := TGStringList.Create;
  SystemLogMsgList.Clear;
end;

finalization

begin
  FreeAndNil(SystemLogMsgList);
end;
