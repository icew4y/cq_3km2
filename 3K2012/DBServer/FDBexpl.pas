...
begin
  FrmNewChr.sub_49BD60(sChrName, sAccount);
  FrmUserSoc.NewChrData(sChrName, sAccount, 0, 0, 0);
end;
...
begin
        ...
        if n8 >= 0 then begin
          if ((ChrRecord.Header.dCreateDate < dt20) and (ChrRecord.Data.Abil.Level <= wLevel1)) or
            ...
            if ClearHumanItem(ChrRecord) then begin
              {$IF DBSMode = 1}
              HumDataDB.Update(g_nClearIndex, @ChrRecord, 5);
              {$ELSE}
  ...
  if sHumName <> '' then FrmDBSrv.DelHum(sHumName);
end;
...
begin
  ...
  sDestChrName := FrmCopyRcd.s2F4;
  sUserId := FrmCopyRcd.s2F8;
  if FrmUserSoc.NewChrData(sDestChrName, sUserId, 0, 0, 0) and
