...
var
  sGameGateIP, sIP: string;
  nGameGatePort: Integer;
begin
  Result := False;
  FillChar(m_RouteInfo, SizeOf(m_RouteInfo), 0);
  m_RouteInfo.sSelGateIP := Trim(EditSelGate.Text);
  if CheckIsIpAddr(m_RouteInfo.sSelGateIP) then begin
    sIP:= m_RouteInfo.sSelGateIP;
  end else begin
    sIP:= HostToIP(m_RouteInfo.sSelGateIP);
  end;
  if not IsIPaddr({m_RouteInfo.sSelGateIP}sIP) then begin
  ...
  if not CheckIsIpAddr(sGameGateIP) then begin
    sGameGateIP:= HostToIP(sGameGateIP);
  end;
  ...
  if not CheckIsIpAddr(sGameGateIP) then sGameGateIP:= HostToIP(sGameGateIP);
