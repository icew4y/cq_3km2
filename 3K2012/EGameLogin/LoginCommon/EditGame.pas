...
begin
  ...
  if FileExists(PChar(g_sMirPath)+m_sLocalGameListName) then begin
    LoadList := Classes.TStringList.Create();
    LoadList.LoadFromFile(PChar(g_sMirPath)+m_sLocalGameListName);
...
begin
  ...
  for I := 0 to ListBoxGame.Items.Count - 1 do begin
    ...
    if ServerInfo <> nil then begin
      sLineText := ServerInfo.ServerName + '|' +
                   ServerInfo.ServerIP + '|' +
                   IntToStr(ServerInfo.ServerPort) +'|' +
                   ServerInfo.ServerNoticeURL + '|' +
                   ServerInfo.ServerHomeURL;
      ServerList.Add(sLineText);
    end;
  end;
  ServerList.SaveToFile(g_sMirPath +m_sLocalGameListName);
  ServerList.Free;
end;
procedure TFrmEditGame.ListBoxGameClick(Sender: TObject);

begin
  if ListBoxGame.ItemIndex < 0  then Exit;
...
begin
  ...
  FrmMain.TimeGetGameList.Enabled := True;
  Close;
end;
