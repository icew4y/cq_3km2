...
begin
  ...
  while (true) do begin
    ...
    if sIPaddr <> '' then begin
      if CheckIsIpAddr(sIPaddr) then begin//检查IP是否合法，如为域名则转换为IP 20100405
        Config.GateRoute[nRouteIdx].Gate[nGateIdx].sIPaddr := sIPaddr;
      end else begin
        sIP:= HostToIP(sIPaddr);//20100405 域名转IP
        if CheckIsIpAddr(sIP) then Config.GateRoute[nRouteIdx].Gate[nGateIdx].sIPaddr := sIP;
      end;
