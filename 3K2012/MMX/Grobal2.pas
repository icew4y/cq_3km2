...
const
  ...
  {$IFEND}
  CLIENT_VERSION_NUMBER = 920110930;//9+客户端版本号 20091025，当小于20080512时，Run网关会把玩家T了，RUN代码中检查自己的G单元的日期(20080512)
  MAXPATHLEN = 255;                               
  DIRPATHLEN = 80;
  MAPNAMELEN = 16;//地图名长度
...
//----------------------------------------------------------------------------
  ...
  CM_SENDLOGINRANDOMID = 179; //发送网关随机标实
...
//------------------------------------------------------------------------------
  ...
  TClientSuitAbility = packed record//客户端套装属性值 20110711
    nMasterAbility: Byte;//主属性
...
{$IFEND}  
 ...
 { TRegInfo = record//20110220 注释
  ...
  TClientConf = record
    ...
    boUseCanDivision      : Boolean;//内挂隔位刺杀是否可用
  end;
