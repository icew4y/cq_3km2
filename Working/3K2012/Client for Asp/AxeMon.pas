...
uses
  Windows, Classes, 
  Grobal2, AbstractCanvas, AbstractTextures, AsphyreTypes,
  CliUtil, ClFunc, magiceff, Actor, ClEvent;
...
uses
   ClMain, SoundUtil, AspWIL, MShare;
   ...
   case m_nCurrentAction of
      ...
      SM_DIGUP: //°È±â ¾øÀ½, SM_DIGUP, ¹æÇâ ¾øÀ½.
         begin
            ...
            end else if (m_btRace = 78) then begin //ÐÞÕýÄ§Áú½ÌÖ÷¼¤»îÐ§¹û By TasNat at: 2012-04-06 19:43:19
...
begin
   if m_nCurrentAction = SM_HIT then begin
      if (frame = 5) and (oldframe <> frame) then begin
        if m_wAppearance = 273 then begin  //Ê¥ÊÞ
...
procedure TCatMon.DrawChr (dsurface: TAsphyreCanvas; dx, dy: integer; blend: Boolean;boFlag:Boolean);
var
  ceff: TColorEffect;
  idx, Wax, Way: Integer;
  d: TAsphyreLockableTexture;
...
begin
  try
    ...
    if BoUseDieEffect and (DieEffectSurface <> nil) then begin
      dsurface.DrawBlend (dx + bx + m_nShiftX,
                          dy + by + m_nShiftY,
                          DieEffectSurface);
    end;
  except
...
procedure TBanyaGuardMon.DrawEff(dsurface: TAsphyreCanvas; dx,
  dy: integer);
begin
  try
    inherited;
    if m_boUseEffect and (n26C <> nil) then begin
      dsurface.DrawBlend (dx + ax + m_nShiftX,dy + ay + m_nShiftY,n26C);
    end;
  except
...
procedure TSkeletonArcherMon.DrawEff(dsurface: TAsphyreCanvas; dx,
  dy: integer);
begin
  try
    inherited;
    if bo260 and (AttackEffectSurface <> nil) then begin
      dsurface.DrawBlend (dx + n264 + m_nShiftX,dy + n268 + m_nShiftY,AttackEffectSurface);
    end;
  except
...
procedure TStoneMonster.DrawEff(dsurface: TAsphyreCanvas; dx,
  dy: integer);
begin
  try
    inherited;
...
procedure TAngel.DrawChr(dsurface: TAsphyreCanvas; dx, dy: integer;
...
//    DrawBlend (dsurface, dx + n270 + m_nShiftX, dy + n274 + m_nShiftY, g_ImgMixSurface, 1);
    ...
    dsurface.DrawBlend (dx + n270 + m_nShiftX, dy + n274 + m_nShiftY, n278);
  end;
  //inherited;
...
procedure TDragonStatue.DrawEff(dsurface: TAsphyreCanvas; dx,
  dy: integer);
begin
  try
  inherited;
  if m_boUseEffect and (EffectSurface <> nil) then begin
    dsurface.DrawBlend (dx + ax + m_nShiftX,dy + ay + m_nShiftY,EffectSurface);
  end;
  except
...
procedure TFireDragon.DrawEff(dsurface: TAsphyreCanvas; dx,
  dy: integer);
begin
  try
  inherited;
  if m_boUseEffect and (n270 <> nil) then begin
    dsurface.DrawBlend (dx + ax + m_nShiftX,dy + ay + m_nShiftY,n270);
  end;
  except
  ...
  try
   ...
   if m_boUseEffect then begin
     case m_nCurrentAction of
       ...
       SM_LIGHTING: begin
         n270 := g_WDragonImages.GetCachedImage (90+ firedir * 10 + m_nEffectFrame, ax, ay);
       end; 
     end;
   end;
   except
...
procedure TWealthAnimalMon.DrawChr(dsurface: TAsphyreCanvas; dx,
...
procedure TWealthAnimalMon.DrawEff(dsurface: TAsphyreCanvas; dx,
  dy: integer);
begin
  try
...
procedure TNewFireDragon.DrawChr(dsurface: TAsphyreCanvas; dx, dy: integer;
  blend, boFlag: Boolean);
var
  ceff: TColorEffect;
  idx, Wax, Way: Integer;
