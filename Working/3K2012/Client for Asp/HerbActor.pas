...
begin
   mimg := GetMonImg (m_wAppearance);
   if (mimg = nil)  then Exit;//ÐÞ¸´AV´íÎóBy TasNat at: 2012-04-07 13:46:28
   
   if (deathframe > 0)
   then begin //(CurrentAction = SM_NOWDEATH) or (CurrentAction = SM_DEATH) then begin
...
begin
  try
    if m_boUseEffect and (AttackEffectSurface <> nil) then
      dsurface.DrawBlend (dx + ax + m_nShiftX,dy + ay + m_nShiftY,AttackEffectSurface);
  except
  ...
  try
   ...
   if GetTickCount - m_dwLoadSurfaceTime > 60 * 1000 then begin
    ...
    if m_BodySurface <> nil then
      dsurface.DrawBlend (dx + m_nPx + m_nShiftX,dy + m_nPy + m_nShiftY,m_BodySurface);
  except
