...
interface
uses
  Windows, SysUtils, Classes, cliutil, AbstractTextures, AspDWinCtl,
  AspWIL, UiWil, Actor, Grobal2, AsphyreDXSounds, IniFiles, Share,Graphics, Rc6, Mars, StrUtils, CnHashTable;
type
//自身信息记录
  TRecinfo = record
    GameSky: string[100];
  ...
  TTimerCommand = (tcNone, tcSoftClose, tcReSelConnect, tcFastQueryChr, tcQueryItemPrice);
  TChrAction = (caWalk, caRun, caHorseRun, caHit, caSpell, caSitdown);
  TConnectionStep = (cnsNone, cnsLogin, cnsSelChr, cnsReSelChr, cnsPlay);
...
//=============药品================
  ...
  TD3DConfig = record
    wScreenWidth, wScreenHeight: Word; //分辨率
    boVSync: Boolean; //垂直同步
		boD3DFormat :Boolean; //文理压缩
    boFullScreen :Boolean; //文理压缩
    btBitCount: Byte; //色深
  end;
  ...
  TItemArrs = array[0..MAXBAGITEMCL-1] of TItemArr;
...
var
  ...
  g_sLoginKey       :string;
  g_sLoginGatePassWord       :string{$if GVersion <> 1} = '123' {$ifend};
  g_boOnePlay       :Boolean = False; //是否第一次点开始游戏
  g_sGameESystem    :String = 'http://127.0.0.1/QKServerList.txt'; //E系统网址　20080603
  g_MapDescList     :TList; //地图注释列表
  //VIEWCHATLINE      :Byte; //文字聊天行数
  g_TzHintList: TList;
  g_RecInfo         :TRecInfo;
  g_D3DConfig       :TD3DConfig =
  	(
    wScreenWidth : 800; //分辨率
    wScreenHeight : 600;
    boVSync : True; //垂直同步
		boD3DFormat : True; //文理压缩
    btBitCount : 32; //色深
    );
  {$if GVersion = 1}
...
(******************************************************************************)
  ...
  {$IFEND}
  g_DrawUseItems: array[0..14] of TDrawEffect;
  g_DrawUseItems1: array[0..14] of TDrawEffect;
  g_DrawHeroUseItems: array[0..14] of TDrawEffect;
...
//签定物品
  ...
  {$IFEND}
  g_boSendOnePack              : Boolean =  {$if GVersion <> 2} not {$ifend} False;
(*******************************************************************************)
...
{******************************************************************************}
//金牛物品发光
    JNItemLightTimeTick: LongWord;
    JNItemLightImgIdx: integer;
    JNHelmetLightTimeTick: LongWord;
    JNHelmetLightImgIdx: Integer;
{******************************************************************************}
//虎威装备发光
    HWItemLightImgIdx: Integer;
    HWItemLightTimeTick: LongWord;
(******************************************************************************)
//主宰装备发光
    ZZItemLightImgIdx: Integer;
    ZZItemLightTimeTick: LongWord;
(******************************************************************************)
   g_pwdimgstr : string;
...
//连击状态栏
  ...
  g_dCheckNumImages: TAsphyreLockableTexture;
...
{--------------------英雄版2007.10.17 添加---------------------------}
  ...
  g_RefuseWHISPER               :Boolean=False;    //拒绝私聊信息
...
//粹练系统20080506
  ...
  //g_RefineDrumItemName          :array[0..6] of string;  //=('魔血石(大)','魔血石(大)','','','','盟重传送石');
  //g_RefineDrumCount             :Integer=6;
  {******************************************************************************}
...
//新盛大内挂 20080624
  ...
  g_dwEditExpFiltrate         :Int64; //过滤经验
...
{******************************************************************************}
  ...
  //g_DXDraw           :TDXDraw;
  g_DWinMan          :TAspDWinManager;
  g_DXSound          :TDXSound;
  g_Sound            :TSoundEngine;

  g_WMainImages      :TAspWMImages;
  g_WMain2Images     :TAspWMImages;
  g_WMain3Images     :TAspWMImages;
  g_WChrSelImages    :TAspWMImages;
  g_WMMapImages      :TAspWMImages;
  g_WHumWingImages   :TAspWMImages;
  g_WHumWing2Images   :TAspWMImages;
  g_WHumWing3Images   :TAspWMImages;

  g_WCboHumWingImages:TAspWMImages;
  g_WCboHumWingImages2:TAspWMImages;
  g_WCboHumWingImages3:TAspWMImages;

  g_WBagItemImages   :TAspWMImages;
  g_WBagItem2Images  :TAspWMImages;
  g_WStateItemImages :TAspWMImages;
  g_WStateItem2Images :TAspWMImages;
  g_WDnItemImages    :TAspWMImages;
  g_WDnItem2Images   :TAspWMImages;
  g_WHumImgImages    :TAspWMImages;
  g_WHum2ImgImages   :TAspWMImages; //20080501
  g_WHum3ImgImages   :TAspWMImages;
  g_WHum4ImgImages   :TAspWMImages;

  g_WHumEx1ImgImages  :TAspWMImages; // 扩展的新的外观
  g_WHumEx2ImgImages  :TAspWMImages; //
  g_WHumEx3ImgImages  :TAspWMImages; //
  g_WHumEx4ImgImages  :TAspWMImages; //
  g_WHumEx5ImgImages  :TAspWMImages; //

  g_WCboHumImgImages :TAspWMImages;
  g_WCboHum3ImgImages :TAspWMImages;
  g_WHairImgImages   :TAspWMImages;
  g_WCboHairImgImages:TAspWMImages;
  g_WWeaponImages    :TAspWMImages;
  g_WCboWeaponImages :TAspWMImages;
  g_WCboWeaponImages3:TAspWMImages;
  g_WWeapon2Images   :TAspWMImages; //20080501
  g_WWeapon3Images   :TAspWMImages; 
  g_WMagIconImages   :TAspWMImages;
  g_WMagIcon2Images  :TAspWMImages;
  g_WNpcImgImages    :TAspWMImages;
  g_WNpc2ImgImages   :TAspWMImages;
  g_WMagicImages     :TAspWMImages;
  g_WMagic2Images    :TAspWMImages;
  g_WMagic3Images    :TAspWMImages;
  g_WMagic4Images    :TAspWMImages;
  g_WMagic5Images    :TAspWMImages;
  g_WMagic6Images    :TAspWMImages;

  g_WMagic7Images    :TAspWMImages;
  g_WMagic7Images16  :TAspWMImages;
  g_WMagic8Images    :TAspWMImages;
  g_WMagic8Images16  :TAspWMImages;
  g_WMagic9Images    :TAspWMImages; // liuzhigang add
  g_WMagic10Images   :TAspWMImages; // liuzhigang add

  g_WEffectImages    :TAspWMImages;
  g_qingqingImages   :TAspWMImages;
  g_TasUiImages      :TAspWMImages;//优先考虑GM定义的文件  By TasNat at: 2012-04-15 13:45:37

  g_WchantkkImages   :TAspWMImages; // liuzhigang

  g_WNewopUIImages   :TAspWMImages;
  g_WDragonImages    :TAspWMImages;
  g_WCboEffectImages :TAspWMImages;
  g_WUI1Images       :TAspWMImages;
  g_WUI3Images       :TAspWMImages;
  g_WStateEffectImages: TAspWMImages;
  g_WUiMainImages    :TUIWMImages;
  g_WWeaponEffectImages:TAspWMImages;
  g_nTmpX, g_nTmpY : Integer;//读界面用

  g_WObjectArr              :array[0..30] of TAspWMImages;//数值越大 支持的 Object素材 越多  2007.10.27
  g_WTilesArr               :array[0..2] of TAspWMImages;
  g_WSMTilesArr             :array[0..2] of TAspWMImages;
  g_WMonImagesArr           :array[0..53] of TAspWMImages;
  g_WMonKuLouImages         :TAspWMImages;    
  ...
  //g_ImgMixSurface           :TAsphyreLockableTexture; //
...
{******************************************************************************}
  ...
  g_SendNoticeWait       :Boolean;//是否点了公告框确定 By TasNat at: 2012-04-19 11:28:12
  g_SendNoticePass       :Boolean;//是否点了公告框确定 By TasNat at: 2012-04-19 11:28:12
  g_ItemArr                    :TItemArrs;//TClientEffecItem;
  g_UseItems                   :array[0..14] of TClientItem;
  ...
  g_boCan4LongHit        :Boolean; //4级刺杀剑术
...
//  g_boCan69Hit           :Boolean;
  ...
  procedure IniTAspWMImagesLib({DDxDraw: TDxDraw});
  procedure UnIniTAspWMImagesLib();
  procedure UnLoadWMImagesLib();
  function  GetTiles(nUnit,nIdx:Integer):TAsphyreLockableTexture;
  function  GetSmTiles(nUnit,nIdx:Integer):TAsphyreLockableTexture;    
  function  GetObjs (nUnit,nIdx:Integer):TAsphyreLockableTexture;
  function  GetObjsEx (nUnit,nIdx:Integer; var px, py: integer): TAsphyreLockableTexture;
  function  GetMonImg (nAppr:Integer):TAspWMImages;
...
implementation
uses FState, ClMain, HUtil32, Menus{$IF GVersion = 1},ComObj{$IFEND};
...
var
   ...
   fhandle: TextFile;
begin
  if not DirectoryExists(g_ParamDir) then
    flname := BugFile
  else
   flname := g_ParamDir+BugFile;
   try
     if ForceDirectories(ExtractFilePath(flname)) then begin
       ...
       WriteLn (fhandle, g_sVerIdent + ':' +TimeToStr(Time) + ' ' + msg);
       CloseFile (fhandle);
     end;
   except
...
procedure LoadWMImagesLib(AOwner: TComponent);
begin
  g_WMainImages        := TAspWMImages.Create(AOwner);
  g_WMain2Images       := TAspWMImages.Create(AOwner);
  g_WMain3Images       := TAspWMImages.Create(AOwner);
  g_WChrSelImages      := TAspWMImages.Create(AOwner);
  g_WMMapImages        := TAspWMImages.Create(AOwner);
  g_WHumWingImages     := TAspWMImages.Create(AOwner);
  g_WHumWing2Images    := TAspWMImages.Create(AOwner);
  g_WHumWing3Images    := TAspWMImages.Create(AOwner);

  g_WBagItemImages     := TAspWMImages.Create(AOwner);
  g_WBagItem2Images    := TAspWMImages.Create(AOwner);
  g_WStateItemImages   := TAspWMImages.Create(AOwner);
  g_WStateItem2Images  := TAspWMImages.Create(AOwner);
  g_WDnItemImages      := TAspWMImages.Create(AOwner);
  g_WDnItem2Images     := TAspWMImages.Create(AOwner);
  g_WHumImgImages      := TAspWMImages.Create(AOwner);
  g_WHum2ImgImages     := TAspWMImages.Create(AOwner); //20080501
  g_WHum3ImgImages     := TAspWMImages.Create(AOwner);
  g_WHum4ImgImages     := TAspWMImages.Create(AOwner);
  g_WCboHumImgImages   := TAspWMImages.Create(AOwner);
  g_WCboHum3ImgImages  := TAspWMImages.Create(AOwner);
  g_WHairImgImages     := TAspWMImages.Create(AOwner);
  g_WCboHairImgImages  := TAspWMImages.Create(AOwner);
  g_WWeaponImages      := TAspWMImages.Create(AOwner);
  g_WCboWeaponImages   := TAspWMImages.Create(AOwner);
  g_WCboWeaponImages3  := TAspWMImages.Create(AOwner);
  g_WWeapon2Images     := TAspWMImages.Create(AOwner); //20080501
  g_WWeapon3Images     := TAspWMImages.Create(AOwner);
  g_WMagIconImages     := TAspWMImages.Create(AOwner);
  g_WMagIcon2Images    := TAspWMImages.Create(AOwner);
  g_WNpcImgImages      := TAspWMImages.Create(AOwner);
  g_WNpc2ImgImages     := TAspWMImages.Create(AOwner);
  g_WMagicImages       := TAspWMImages.Create(AOwner);
  g_WMagic2Images      := TAspWMImages.Create(AOwner);
  g_WMagic3Images      := TAspWMImages.Create(AOwner);
  g_WMagic4Images      := TAspWMImages.Create(AOwner);    //2007.10.28
  g_WMagic5Images      := TAspWMImages.Create(AOwner);   //207.11.29
  g_WMagic6Images      := TAspWMImages.Create(AOwner);   //207.11.29

  g_WMagic7Images      := TAspWMImages.Create(AOwner);
  g_WMagic7Images16    := TAspWMImages.Create(AOwner);
  g_WMagic8Images      := TAspWMImages.Create(AOwner);
  g_WMagic8Images16    := TAspWMImages.Create(AOwner);

  g_WMagic9Images      := TAspWMImages.Create(AOwner);
  g_WMagic10Images     := TAspWMImages.Create(AOwner);
  g_WMonKuLouImages    := TAspWMImages.Create(AOwner);
  g_WEffectImages      := TAspWMImages.Create(AOwner);
  g_qingqingImages     := TAspWMImages.Create(AOwner);
  g_TasUiImages        := TAspWMImages.Create(AOwner);
  g_WNewopUIImages     := TAspWMImages.Create(AOwner);
  g_WDragonImages      := TAspWMImages.Create(AOwner);
  g_WUiMainImages      := TUIWMImages.Create;
  
  g_WWeaponEffectImages:= TAspWMImages.Create(AOwner);

  g_WCboHumWingImages  := TAspWMImages.Create(AOwner);
  g_WCboHumWingImages2 := TAspWMImages.Create(AOwner);
  g_WCboHumWingImages3 := TAspWMImages.Create(AOwner);
  g_WCboEffectImages   := TAspWMImages.Create(AOwner);
  g_WUI1Images := TAspWMImages.Create(AOwner);
  g_WUI3Images := TAspWMImages.Create(AOwner);
  g_WStateEffectImages := TAspWMImages.Create(AOwner);

  g_WchantkkImages := TAspWMImages.Create(AOwner); // liuzhigang add
  g_WMainImages.FileName  := g_ParamDir+MAINIMAGEFILE;
  ...
  g_WBagItemImages.FileName := g_ParamDir+BAGITEMIMAGESFILE;
  ...
  g_WStateItemImages.FileName := g_ParamDir+STATEITEMIMAGESFILE;
  ...
  g_WDnItemImages.FileName:=g_ParamDir+DNITEMIMAGESFILE;
  ...
  g_WWeapon2Images.FileName:=g_ParamDir+WEAPON2IMAGESFILE; //20080501
  ...
  g_TasUiImages.FileName:=g_ParamDir+ TasUiFILE;
  g_TasUiImages.LibType:=ltUseCache;

  g_WchantkkImages.FileName := g_ParamDir+chantkkFILE;
  g_WchantkkImages.LibType:=ltUseCache;


  g_WNewopUIImages.FileName := g_ParamDir+NEWOPUIFILE;
  g_WNewopUIImages.LibType:=ltUseCache;
...
begin
  try
  for I := 0 to g_WilList.Count - 1 do
    TAspWMImages(g_WilList[I]).Initialize;
...
begin
  for I := 0 to g_WilList.Count - 1 do
    TAspWMImages(g_WilList[I]).Finalize;
...
begin
  for I:= Low(g_WObjectArr) to (High(g_WObjectArr)) do begin
    ...
    if g_WObjectArr[I] = nil then begin
      g_WObjectArr[I]:=TAspWMImages.Create(nil);
...
begin
  for I:=Low(g_WTilesArr) to High(g_WTilesArr) do begin
    ...
    if g_WTilesArr[I] = nil then begin
      g_WTilesArr[I]:=TAspWMImages.Create(nil);
...
begin
  for I:=Low(g_WSmTilesArr) to High(g_WSmTilesArr) do begin
    ...
    //if not FileExists(sFileName) then Continue;
    if g_WSmTilesArr[I] = nil then begin
      g_WSmTilesArr[I]:=TAspWMImages.Create(nil);
...
begin
  for I:= Low(g_WMonImagesArr) to (High(g_WMonImagesArr)) do begin
    if g_WMonImagesArr[I] = nil then begin
      ...
      g_WMonImagesArr[I]:=TAspWMImages.Create(nil);
...
begin
  ...
  try
   ...
   // end;
    ...
    g_RefuseWHISPER:=Ini.ReadBool('OtherParm', 'RefuseWhisper', False);
    FrmDlg.RefuseWHISPERLevel:=Ini.ReadInteger('OtherParm', 'RefuseWhisperLevel', 9999);
...
begin
  ...
  {$IFEND}
  Ini.WriteBool('OtherParm', 'RefuseWhisper', g_RefuseWHISPER);
  Ini.WriteInteger('OtherParm', 'RefuseWhisperLevel', FrmDlg.RefuseWHISPERLevel);
  Ini.UpdateFile;
  Ini.Free;
end;
{******************************************************************************}
        ...
        for I:=0 to g_TzHintList.Count - 1 do begin
          ...
          if TzHintInfo <> nil then begin
            ...
            if Pos(Items, TzHintInfo.sTzItems) > 0 then begin
              if TzHintInfo.btJob in [0..2] then begin //其他职业通过
                case Who of
                  1: begin
                    if g_MySelf <> nil then begin
                      if g_MySelf.m_btJob <> TzHintInfo.btJob then Continue;
                    end;
                  end;
                  2: begin
                    if g_HeroSelf <> nil then begin
                      if g_HeroSelf.m_btJob <> TzHintInfo.btJob then Continue;
                    end;
                  end;
                  3: begin
                    if FrmDlg.UserState1.btJob <> TzHintInfo.btJob then Continue;
                  end;
                  else Continue;
                end;
              end;
  ...
  if TzInfo.sMemo <> '' then begin
    ...
    try
      ...
      for II:=0 to TempStringList.Count-1 do  begin
        sLine := TempStringList[II];
      	if Pos ('%', sLine) > 0 then begin
          if Pos ('%ng', sLine) > 0 then begin
            nIncNG := 0;
            case Who of
              2: begin
                ...
                Result := Result + AnsiReplaceText(sLine, '%ng', IntToStr(nIncNG)) + '\';
              end;
              3: begin
                for I:= Low(THumanUseItems) to High(THumanUseItems) do begin
          ...
          if Pos ('%fb', sLine) > 0 then begin
            ...
            case Who of
              2: begin
                ...
                Result := Result + AnsiReplaceText(sLine, '%fb', IntToStr(nIncNG)) + '\';
              end;
              3: begin
                for I:= Low(THumanUseItems) to High(THumanUseItems) do begin
                  if FrmDlg.UserState1.UseItems[I].s.Name <> '' then begin
          ...
          if Pos ('%xx', sLine) > 0 then begin  //吸血
            nIncNG := 0;
            case Who of
              2: begin
                for I:= Low(THumanUseItems) to High(THumanUseItems) do begin
                ...
                Result := Result + AnsiReplaceText(sLine, '%xx', IntToStr(nIncNG)) + '\';
              end;
              3: begin
                for I:= Low(THumanUseItems) to High(THumanUseItems) do begin
                  if FrmDlg.UserState1.UseItems[I].s.Name <> '' then begin
          ...
          if Pos ('%ns', sLine) > 0 then begin  //内伤等级
            nIncNG := 0;
            case Who of
              2: begin
                for I:= Low(THumanUseItems) to High(THumanUseItems) do begin
                ...
                Result := Result + AnsiReplaceText(sLine, '%ns', IntToStr(nIncNG)) + '\';
              end;
              3: begin
                for I:= Low(THumanUseItems) to High(THumanUseItems) do begin
                  if FrmDlg.UserState1.UseItems[I].s.Name <> '' then begin
          ...
          if Pos ('%zsx', sLine) > 0 then begin  //主属性
          	nIncNG := 0;
            case Who of
            	1: begin
                  ...
                  if nMasterAbility > 0 then begin
                    ...
                    if nIncNG > 0 then
                      Result := Result + Format('Lv%d\', [nIncNG]) + AnsiReplaceText(sLine, '%zsx', IntToStr(nMasterAbility)) + '\'
                    else  Result := Result + AnsiReplaceText(sLine, '%zsx', IntToStr(nMasterAbility)) + '\'
          ...
          if Pos ('%jm', sLine) > 0 then begin  //召唤巨魔等级
            nIncNG := 0;
            case Who of
              3: begin
                nIncNG := FrmDlg.UserState1.nCallTrollLevel;
                Result := Result + AnsiReplaceText(sLine, '%jm', IntToStr(nIncNG)) + '\';
              end;
              1: begin
          ...
          if Pos ('%hjwl', sLine) > 0 then begin  //合击威力
          	nIncNG := 0;
            case Who of
            	1: begin
              	if g_MySelfSuitAbility.nIncDragon > 0 then
	              	Result := Result + AnsiReplaceText(sLine, '%hjwl', IntToStr(g_MySelfSuitAbility.nIncDragon)) + '\'
              end;
              2: begin
                if g_MyHeroSuitAbility.nIncDragon > 0 then
									Result := Result + AnsiReplaceText(sLine, '%hjwl', IntToStr(g_MyHeroSuitAbility.nIncDragon)) + '\'

              end;
              3: begin
                if FrmDlg.UserState1.SuitAbility.nIncDragon > 0 then
                	Result := Result + AnsiReplaceText(sLine, '%hjwl', IntToStr(FrmDlg.UserState1.SuitAbility.nIncDragon)) + '\'

              end;
            end;
          {$IFEND}
          end else Result := Result + sLine + '\';

        end else Result := Result + sLine + '\'// else TempStringList[II] := TempStringList[II] + '\';
      end;
...
begin
  ...
  if g_TitleDesc.Count > 0 then begin
    ...
    if I > -1 then begin
      ItemDesc := pTTitleDesc(g_TitleDesc.Objects[I]);
      if ItemDesc <> nil then Result := ItemDesc.sTitleDesc;
    end;
  end;
end;
...
begin
  ...
  if g_TitleDesc.Count > 0 then begin
    ...
    if I > -1 then begin
      ...
      if TitleDesc <> nil then Result := TitleDesc.sTitleDesc;//sNewStateTitleDesc;
    end;
  end;
end;
{$IFEND}
