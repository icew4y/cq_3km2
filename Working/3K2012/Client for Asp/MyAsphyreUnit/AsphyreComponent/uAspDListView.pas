...
uses Types, Controls, Classes, Graphics, Math, AbstractCanvas, AbstractTextures,
     AsphyreTextureFonts, AspDWinCtl, AspWil, uAspDScrollBar, uAspDControls;
...
type
  ...
  TAspDListView = class(TAspDControl)
  private
    FLines: TList;
    ColWidths: array of Integer;
    FColCount: Integer;
    FOnListItemClick: TOnListItem;
    FHScrollBar: TAspDScrollBar;            //0x34
...
procedure Register;
begin
   RegisterComponents('AspMirGame', [TAspDListView]);
end;
...
function TAspDListView.Add: TDListItem;
begin
  Result := TDListItem.Create;
  FLines.Add(Result);
end;

procedure TAspDListView.Clear;
var
  I: Integer;
begin
...
function TAspDListView.Click(X, Y: integer): Boolean;
var
  I: Integer;
  nWidth: Integer;
  nLeft: Integer;
...
constructor TAspDListView.Create(AOwner: TComponent);
begin
...
destructor TAspDListView.Destroy;
var
  I: Integer;
begin
  for I := 0 to FLines.Count - 1 do
...
procedure TAspDListView.DirectPaint(dsurface: TAsphyreCanvas);
var
...
procedure TAspDListView.DoScroll(Value: Integer);
var
  I: Integer;
  vRect: TRect;
  ARect: TRect;
  ...
  if Value <> 0 then begin
    ...
    if DControls.Count > 0 then begin//20080629
      for i:=0 to DControls.Count-1 do begin
        D := DControls[I];
        if D is TAspDScrollBar then continue;
        D.GTop := D.GTop + Value;
        ARect := D.ClientRect;
...
procedure TAspDListView.FillItemMouse;
var
  I, II: Integer;
  ListItem: TDListItem;
begin
...
function TAspDListView.GetColWidth(Index: Integer): Integer;
begin
  Result := ColWidths[Index];
end;

function TAspDListView.GetCount: Integer;
begin
  Result := FLines.Count;
end;

function TAspDListView.GetViewItem(Index: Integer): TDListItem;
begin
  Result := FLines[Index];
end;

function TAspDListView.MouseDown(Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer): Boolean;
var
  I: Integer;
  nWidth: Integer;
...
function TAspDListView.MouseMove(Shift: TShiftState; X, Y: Integer): Boolean;
var
  I: Integer;
  nWidth: Integer;
  nLeft: Integer;
...
function TAspDListView.MouseUp(Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer): Boolean;
var
  I: Integer;
  nWidth: Integer;
...
procedure TAspDListView.SetColCount(Value: Integer);
var
  I: Integer;
begin
...
procedure TAspDListView.SetColWidth(Index, Value: Integer);
begin
...
procedure TAspDListView.SetHScrollBar(Value: TAspDScrollBar);
begin
  FHScrollBar := Value;
end;
