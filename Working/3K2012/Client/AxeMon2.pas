...
interface
uses Windows, Actor, AxeMon, DxDraws, Grobal2, MShare, cliUtil, clEvent, magiceff;
...
type
   ...
   protected
     m_LightSurface      :TDirectDrawSurface;
...
//ÔÂÁé
   TFairyMonster = class (TActor)  //ÔÂÁé
   protected
     m_LightSurface: TDirectDrawSurface;
...
implementation
uses ClMain, ClFunc, SoundUtil, Wil;
{ TSnowyHousecarl }
...
procedure TSnowyGuards.DrawChr(dsurface: TDirectDrawSurface; dx,
...
procedure TSnowyGuards.DrawEff(dsurface: TDirectDrawSurface; dx,
  dy: integer);
begin
  try
...
procedure TSnowyBE.DrawChr(dsurface: TDirectDrawSurface; dx, dy: integer;
...
procedure TFoxRed.DrawEff(dsurface: TDirectDrawSurface; dx, dy: integer);
begin
  try
    if m_boUseEffect and (AttackEffectSurface <> nil) then begin
      DrawBlend (dsurface,
...
procedure TFoxBall.DrawEff(dsurface: TDirectDrawSurface; dx, dy: integer);
begin
...
procedure TFoxStone.DrawEff(dsurface: TDirectDrawSurface; dx, dy: integer);
begin
    if BoUseDieEffect and (DieEffectSurface <> nil) then begin
      DrawBlend (dsurface,
              dx + bx + m_nShiftX,
...
procedure TFairyMonster.DrawChr (dsurface: TDirectDrawSurface; dx, dy: integer;
   ...
   if m_BodySurface <> nil then
    ...
    if not m_boDeath then begin  //ËÀÍö²»ÏÔÊ¾
      if m_nState and $02000000 <> 0 then begin //Íò½£¹é×Ú»÷ÖÐ×´Ì¬
          ...
          if d <> nil then
          	DrawBlend (dsurface, dx + ax + m_nShiftX, dy + ay + m_nShiftY, d, 200);
      end;
      if m_nState and $00004000 <> 0 then begin //¶¨Éí×´Ì¬
          idx := 1080 + (m_nGenAniCount mod 8);
          d := g_WMagic10Images.GetCachedImage(idx, ax, ay);
...
{ TTigerMonster }
procedure TTigerMonster.CalcActorFrame;
var
  pm: PTMonsterAction;
  Meff:TMagicEff;
begin
