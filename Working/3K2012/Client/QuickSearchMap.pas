...
uses
  Windows, Classes, SysUtils, Grobal2, MapUnit;
...
type
  ...
  TQuickSearchMapHeader = packed record// –ﬁ∏ƒMAPÕ∑Ω·ππ
    Width: Word;//øÌ∂»       2
    Height: Word;//∏ﬂ∂»       2
    Title:array[0..15] of Char; //±ÍÃ‚      16
    UpdateDate: TDateTime;//∏¸–¬»’∆⁄     8
    Logo:Byte;//±Í ∂(–¬µƒ∏Ò ΩŒ™02)          1
    Reserved:array[0..22] of Char;  //±£¡Ù      23
  end;
    ...
    if fhandle > 0 then begin
      ...
      for i := lx to rx - 1 do begin
        if (i >= 0) and (i < header.Width) then begin
          FileSeek(fhandle, SizeOf(TQuickSearchMapHeader) + (aline * i) + (SizeOf(TQuickSearchMapInfo) * ty), 0);
          Count:=(by - ty);
