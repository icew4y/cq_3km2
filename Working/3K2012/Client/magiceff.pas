...
{******************************************************************************}
  ...
  { THit4Effects = class (TObjectEffects) //断岳斩
   ...
   TFoxRedDrawEffect = class (TMagicEff)//Size 0xCC
     btAddImage:Byte;
   public
      constructor Create(XX,YY:Integer;WmImage:TWMImages;effbase,nX:Integer;frmTime:LongWord;AddImage: Byte);
      function  Run: Boolean; override;
      procedure DrawEff (surface: TDirectDrawSurface); override;
   end;
   ...
   procedure GetEffectBase (mag, mtype: integer; var wimg: TWMImages; var idx: integer; btdir, btEffectLevelEx: Byte);
   ...
   if m_boActive and ((Abs(FlyX-fireX) > 15) or (Abs(FlyY-fireY) > 15) or FixedEffect) then begin
      ...
      if not FixedEffect then begin
        ErrorCode := 3;
         ...
         {$IF M2Version <> 2}
         if ImgLib = g_WMagic5Images then begin
         ...
         end else if ImgLib = g_WMagic10Images then begin
           case MagExplosionBase of //技能坐标错位
             380: begin //冰霜雪雨
               case img of
                 380..389: begin
      ...
      if (FireNodes[i].FireNumber <= FIREGUNFRAME) and (FireNodes[i].FireNumber > 0) then begin
         ...
         if d <> nil then begin
            ...
            if (firex <> prx) or (firey <> pry) then begin
               prx := firex;
               pry := firey;
               DrawBlend (surface, firex, firey, d, 120);
            end;
         end;
      end;
   end;
...
begin
  try
   ...
   if d <> nil then begin
     ...
     if boC8 then begin
       DrawBlend (surface,nRx + nPx - UNITX div 2,nRy + nPy - UNITY div 2,d, 120);
     end else begin
...
procedure TFireDragonEffect.DrawEff(surface: TDirectDrawSurface);
var
  img: integer;
  d: TDirectDrawSurface;
  shx, shy: integer;
...
procedure TFireFixedEffect.DrawEff(surface: TDirectDrawSurface);
var
  img: integer;
  d: TDirectDrawSurface;
  shx, shy: integer;
...
procedure TJNExploBujaukEffect.DrawEff(surface: TDirectDrawSurface);
var
   img: integer;
   d: TDirectDrawSurface;
   shx, shy: integer;
...
procedure TSYZBujaukEffect.DrawEff(surface: TDirectDrawSurface);
var
   img: integer;
   d: TDirectDrawSurface;
   shx, shy: integer;
...
procedure THXJBujaukEffect.DrawEff(surface: TDirectDrawSurface);
var
   img: integer;
   d: TDirectDrawSurface;
   shx, shy: integer;
...
constructor THit4Effects.Create(ObjectiD2: TObject; WmImage: TWMImages;
...
procedure TMagicEffDir8.DrawEff(surface: TDirectDrawSurface);
var
  img: integer;
  d: TDirectDrawSurface;
  shx, shy: integer;
...
constructor TFoxRedDrawEffect.Create(XX, YY: Integer; WmImage: TWMImages;
...
procedure TFoxRedDrawEffect.DrawEff(surface: TDirectDrawSurface);
var
   d: TDirectDrawSurface;
   ...
   d := ImgLib.GetCachedImage (EffectBase + curframe, nPx, nPy);
    if d <> nil then begin
      ...
      if btAddImage > 0 then begin
        ...
        if d <> nil then begin
          DrawBlend (surface,nRx + nPx1 - UNITX div 2,nRy + nPy1 - UNITY div 2,d, 120);
        end;
      end;
    end;
  except
...
procedure TIceRainEffect.DrawEff(surface: TDirectDrawSurface);
var
  img: integer;
  d: TDirectDrawSurface;
  shx, shy: integer;
    ...
    if m_boActive and ((Abs(FlyX-fireX) > 15) or (Abs(FlyY-fireY) > 15) or FixedEffect) then begin
      ...
      if d <> nil then begin
        DrawBlend (surface, FlyX + px - UNITX div 2,
                   FlyY + py - UNITY div 2, d, 150 );
        DrawBlend (surface, FlyX + px - UNITX div 2-n14,
                   FlyY + py - UNITY div 2-n14, d, 150 );
        DrawBlend (surface, FlyX + px - UNITX div 2+n14,
                   FlyY + py - UNITY div 2-n14, d, 150 );
        DrawBlend (surface, FlyX + px - UNITX div 2-n14,
                   FlyY + py - UNITY div 2+n14, d, 150 );
...
procedure TGroupThuderEffect.DrawEff(surface: TDirectDrawSurface);
var
  img, px, py: integer;
  d: TDirectDrawSurface;
begin
