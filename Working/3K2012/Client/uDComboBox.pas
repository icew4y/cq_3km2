...
interface
uses DWinCtl, Classes, Graphics, uDPopupMenu, DXDraws, Controls, Math, Windows;
...
procedure Register;
begin
  RegisterComponents('MirGame', [TDCombobox]);
end;
...
begin
  ...
  MainMenu := TDPopupMenu.Create(Self);
...
function TDCombobox.GetText: string;
begin
  Result := Caption;
end;
...
procedure TDCombobox.SetText(Value: string);
begin
  Caption := Value;
end;
...
begin
  if FBackground then
    dsurface.FastFillRect(ClientRect, FBackgroundColor);
  if FBorder then
    dsurface.FastFrameRect(ClientRect, FBorderCurrColor);
  ...
  if Caption <> '' then begin
    case FAlignment of
      ...
      taRightJustify: nX := Max(SurfaceX(GLeft), SurfaceX(GLeft + (GWidth - 16 - MainForm.Canvas.TextWidth(Caption))));
      taCenter: nX := SurfaceX(GLeft) + (GWidth - 16 - MainForm.Canvas.TextWidth(Caption)) div 2;
    end;
    dsurface.TextOut(nX,
