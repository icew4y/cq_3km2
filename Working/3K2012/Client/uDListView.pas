...
interface
uses Types, Windows, Controls, Classes, Graphics, Math, DXDraws, DWinCtl, Wil, uDScrollBar, uDControls;
...
begin
  if Assigned (OnDirectPaint) then
    ...
    for I := nIndex to FLines.Count - 1 do begin
      ...
      for II := 0 to nCount - 1 do begin
        ...
        if ViewItem.Caption <> '' then begin
          ...
          if Font.Bold then
            DSurface.BoldTextOut(SurfaceX(nLeft), SurfaceY(nTop) + (FHScrollBar.ItemHeight - MainForm.Canvas.TextHeight('0')) div 2, Font.Color, Font.BColor, ViewItem.Caption)
          else
            DSurface.TextOut(SurfaceX(nLeft), SurfaceY(nTop) + (FHScrollBar.ItemHeight - MainForm.Canvas.TextHeight('0')) div 2, Font.Color, ViewItem.Caption);
    ...
    //Surface.Draw(vbRect.GLeft, vbRect.GTop, PaintRect, Data, Transparent);
    //DSurface.Draw(vbRect.GLeft, vbRect.GTop, PaintRect, DSurface, True);
    //Data.Free;
    //DSurface.Free;  
  end;
