...
interface
uses DWinCtl, Classes, Graphics, DXDraws, Controls, Menus, SysUtils, Windows;
...
procedure TDPopupMenu.DirectPaint(dsurface: TDirectDrawSurface);
var
...
{------------------------------------------------------------------------------}
  dsurface.FastFillRect(ClientRect, FColors.Background);
  dsurface.FastFrameRect(ClientRect, FColors.Border);
{------------------------------------------------------------------------------}
    ...
    for I := 0 to FItems.Count - 1 do begin
      ...
      if Items[I].Visible then begin
        ...
        if FItems[I] = '-' then begin
          ...
          rc.Bottom := rc.Top + 1;
          dsurface.FastFrameRect(rc, FColors.Line);
        end else begin
          if FMoveItemIndex = nIndex then begin
            dsurface.FastFillRect(rc, FColors.Hot);
            dsurface.TextOut(rc.Left, rc.Top + (FItemSize - MainForm.Canvas.TextHeight('Pp')) div 2, CColor, FItems[I]);
          end else begin
            dsurface.TextOut(rc.Left, rc.Top + (FItemSize - MainForm.Canvas.TextHeight('Pp')) div 2, CColor, FItems[I]);
          end;
        end;
