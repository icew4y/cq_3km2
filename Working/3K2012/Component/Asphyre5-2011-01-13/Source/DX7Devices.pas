...
//---------------------------------------------------------------------------
uses
  Windows, DirectDraw7, Direct3D7, Classes, AbstractDevices, AsphyreTypes;
...
begin
  ...
  if (not Result) then
    ...
    if BackBuffer <> nil then begin
      FillChar(BackDesc, SizeOf(TDDSurfaceDesc2), 0);
      BackDesc.dwSize := SizeOf(TDDSurfaceDesc2);
      if Succeeded(BackBuffer.Lock(nil, BackDesc, 0, 0)) then begin
        case BackDesc.ddpfPixelFormat.dwRGBBitCount of
          16: begin
              if (BackDesc.ddpfPixelFormat.dwRBitMask = $F800) and (BackDesc.ddpfPixelFormat.dwGBitMask = $07E0) and (BackDesc.ddpfPixelFormat.dwBBitMask = $001F) then
                PixelFormat := apf_R5G6B5
              else
                if (BackDesc.ddpfPixelFormat.dwRBitMask = $7C00) and (BackDesc.ddpfPixelFormat.dwGBitMask = $03E0) and (BackDesc.ddpfPixelFormat.dwBBitMask = $001F) then
                PixelFormat := apf_X1R5G5B5
              else
                PixelFormat := apf_A1R5G5B5;
            end;
          24: begin
              if (BackDesc.ddpfPixelFormat.dwRBitMask = $FF0000) and (BackDesc.ddpfPixelFormat.dwGBitMask = $00FF00) and (BackDesc.ddpfPixelFormat.dwBBitMask = $0000FF) then
                PixelFormat := apf_R8G8B8;
            end;
          32: begin
              if (BackDesc.ddpfPixelFormat.dwRBitMask = $FF0000) and (BackDesc.ddpfPixelFormat.dwGBitMask = $00FF00) and (BackDesc.ddpfPixelFormat.dwBBitMask = $0000FF) then
                if (BackDesc.ddpfPixelFormat.dwRGBAlphaBitMask <> $000000) then
                  PixelFormat := apf_A8R8G8B8
                else
                  PixelFormat := apf_X8R8G8B8;
            end;
        end;
      end;
...
function TDX7Device.InitDevice(): Integer;
var
  Guid: PGuid;
...
begin
  Result := 0;
  boResult := (DirectDraw = nil) and (Direct3D = nil) and (Device7 = nil);
  if (not boResult) then Exit;
 ...
 // Step 2. Create DirectDraw interface.
  boResult := Succeeded(DirectDrawCreateEx(Guid, DirectDraw, IID_IDirectDraw7, nil));
 ...
 // Step 3. Create Direct3D interface.
  boResult := Succeeded(DirectDraw.QueryInterface(IID_IDirect3D7, Direct3D));
  if (not boResult) then
  begin
    DirectDraw := nil;
 ...
 // Step 9. Create Direct3D device for 3D rendering.
  boResult := Succeeded(Direct3D.CreateDevice(IID_IDirect3DHALDevice, FBackBuffer,
    Device7));
  if (not boResult) then
  begin
    FBackBuffer := nil;
...
function TDX7Device.LockBackBuffer(out Bits: Pointer;
...
begin
  ...
  Result := False;
  if BackBuffer <> nil then begin
    FillChar(fDesc, SizeOf(TDDSurfaceDesc2), 0);
    fDesc.dwSize := SizeOf(TDDSurfaceDesc2);
    if Succeeded(BackBuffer.Lock(nil, fDesc, 0, 0)) then begin
      Bits := fDesc.lpSurface;
      Pitch := fDesc.lPitch;
...
function TDX7Device.UnLockBackBuffer(): Boolean;
begin
  Result := False;
  if BackBuffer <> nil then begin
    Result := Succeeded(BackBuffer.Unlock(nil));
  end;
end;
