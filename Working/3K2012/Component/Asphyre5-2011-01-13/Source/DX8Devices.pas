...
procedure TDX8Device.FindTextureFormats(Adapter: Integer; BackFormat: TD3DFormat);
var
  FormatNo: Integer;
  AFormat: TD3DFormat;
...
//   ModeCount:= Direct3D.GetAdapterModeCount(Adapter, Format); //shj d9
...
{$IFDEF NoWindowChanges}
  Flags2 := Flags2 or D3DCREATE_NOWINDOWCHANGES;
{$ENDIF}

{$IFDEF PreserveFPU}
  Flags2 := Flags2 or D3DCREATE_FPU_PRESERVE;
{$ENDIF}

{$IFDEF EnableMultithread}
  Flags2 := Flags2 or D3DCREATE_MULTITHREADED;
{$ENDIF}
  
  case VertexProcessing of //默认为 vptHardware
    vptMixed:
      if Failed(Direct3D.CreateDevice(Adapter, D3DDEVTYPE_HAL,
        WindowHandle, Flags2 or D3DCREATE_MIXED_VERTEXPROCESSING, Params8,
        Device8)) then
        Result := -13;
    ...
    vptHardware:
      begin
        if Failed(Direct3D.CreateDevice(Adapter, D3DDEVTYPE_HAL,
          WindowHandle, Flags2 or D3DCREATE_HARDWARE_VERTEXPROCESSING, Params8,
          Device8)) then begin
    ...
    lSoft:
      if Failed(Direct3D.CreateDevice(Adapter, D3DDEVTYPE_HAL,
        WindowHandle, Flags2 or D3DCREATE_SOFTWARE_VERTEXPROCESSING, Params8,
        Device8)) then begin
          if Failed(Direct3D.CreateDevice(Adapter, D3DDEVTYPE_HAL,
        WindowHandle, D3DCREATE_SOFTWARE_VERTEXPROCESSING, Params8,
        Device8)) then
          Result := -16;
        end;
  end;
