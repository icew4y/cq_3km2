...
//---------------------------------------------------------------------------
uses
  Windows, Direct3D9, AbstractCanvas, Vectors2, Matrices3,
...
//---------------------------------------------------------------------------
type
  TDrawingMode = (dmUnknown, dmPoints, dmLines, dmTriangles, dmTexMap);
...
procedure TDX9Canvas.ResetDeviceStates();
begin
  FVertexCount := 0;
  FIndexCount := 0;
  FPrimitives := 0;
...
{
DDRS_ZENABLE 用于控制深度缓存的状态。它
的状态是用一个枚举类型来表示的:D3DZB_TRUE、D3DZB_FALSE和D3DZB_USEW。
在D3D中z-buffering 是对z轴上的信息起作用,
所以,如果你需要表示的是2D信息的话,
可以用pd3dDevice->SetRenderState( D3DRS_ZENABLE, D3DZB_FALSE );
关闭z-buffering。比如游戏中的UI,需要用到一些控制面板之类的DD,
这个时候就是需要关闭它的。这个渲染参数的默认值是跟 D3DPRESENT_PARAMETERS
中的EnableAutoDepthStencil的值是相关联的;
如果这个值是TRUE,那么渲染状态就是 D3DZB_TRUE,反之为D3DZB_FALSE。

D3DRS_SHADEMODE 这个参数控制的是阴影模
}

  with Device9 do
  begin
   // Disable 3D fancy stuff.           关闭3D相关
...
{$ENDIF}
   ...
   // Default alpha-blending behavior          通道混合
...
begin
  case Effect of
    ...
    deGrayscaleX:
      with Device9 do
      begin
        SetRenderState(D3DRS_ALPHABLENDENABLE, Integer(False));
        SetRenderState(D3DRS_TextureFactor, D3DCOLOR_ARGB(129, 160, 15, 180));
        SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_DOTPRODUCT3);
        SetTextureStageState(0, D3DTSS_COLORARG2, D3DTA_TFACTOR);
      end;
...
begin
  ...
  FFlushTexture := (DrawingMode <> dmTexMap) or (CachedTex <> Texture); //增加
  if (NeedReset) then
  begin
    Flush();
...
var
  ...
  i: Integer;
begin
  //FSetTexture:=True;//ActiveTex <> nil;
  //ActiveTex:=nil;
...
var
  Entry: PVertexRecord;
begin
  RequestCache(dmTexMap, 4, 6, Effect, ActiveTex);
