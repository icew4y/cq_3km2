...
uses HUtil32, MudUtil, CreateChr, FDBexpl, viewrcd, EditRcd, UsrSoc, DBShare;
...
begin
  if Key = #13 then begin
    ...
    if sAccount <> '' then begin
      ...
      try
        if {$IF DBSMode = 1}HumChrDB.Open{$ELSE}HumChrDB.OpenEx{$IFEND} then begin
          ...
          if ChrList.Count > 0 then begin//20090101
            for i := 0 to ChrList.Count - 1 do begin
              ...
              if HumChrDB.GetBy(pTQuickID(ChrList.Objects[i]).sChrName, HumDBRecord) then begin
                if CbShowDelChr.Checked then RefChrGrid(nIndex, HumDBRecord)
                else if not HumDBRecord.boDeleted then RefChrGrid(nIndex, HumDBRecord);
              end;
              {$ELSE}
              nIndex := pTQuickID(ChrList.Objects[i]).nIndex;
...
begin
  ...
  try
    ...
    if HumChrDB.Open then begin
      nIndex := HumChrDB.Get(s64, HumDBRecord);
      if nIndex >= 0 then begin
        if CbShowDelChr.Checked then RefChrGrid(nIndex, HumDBRecord)
        else if not HumDBRecord.boDeleted then RefChrGrid(nIndex, HumDBRecord);
      end;
      ...
      if HumChrDB.FindByName(sChrName, ChrList) > 0 then begin
        if ChrList.Count > 0 then begin//20090101
          for i := 0 to ChrList.Count - 1 do begin
            ...
            if HumChrDB.GetBy(ChrList.Strings[I], HumDBRecord) then begin
              if CbShowDelChr.Checked then RefChrGrid(nIndex, HumDBRecord)
              else if not HumDBRecord.boDeleted then RefChrGrid(nIndex, HumDBRecord);
            end;
            {$ELSE}
...
begin
  ...
  try
    if {$IF DBSMode = 1}HumDataDB.Open{$ELSE}HumDataDB.OpenEx{$IFEND} then begin
      ...
      if nC >= 0 then begin
        if {$IF DBSMode = 1}HumDataDB.Get(nC, @ChrRecord) >= 0{$ELSE}HumDataDB.Get(nC, ChrRecord) >= 0{$IFEND} then begin
...
var
  ...
  nIndex, I, K: Integer;
  HumRecord: THumInfo;
  HumanRCD,HeroChrRecord: THumDataInfo;
  ...
  try
    ...
    if MessageDlg('是否确认禁用人物 ' + sChrName + ' ？', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
      try
        if HumChrDB.Open then begin
          {$IF DBSMode = 1}
          nIndex := HumChrDB.Get(sChrName, HumRecord);
          if nIndex >= 0 then begin
  ...
  //--------------------------20080220--------------------------------------------
        if HumDataDB.Open then begin
          ...
          if nIndex >= 0 then begin//20090106
            ...
            {$IFEND}
            if HumanRCD.Data.boHasHero or HumanRCD.Data.boIsHero then begin//有英雄,则把英雄也禁止 20081005
              if HumDataDB.count > 0 then begin//20101012 修改
                ChrList := TStringList.Create;
                try
                  if HumChrDB.Open then begin
                    if HumChrDB.FindByAccount(HumanRCD.Data.sAccount, ChrList) > 0 then begin
                      K:=0;
                      for i := 0 to ChrList.Count - 1 do begin
                ...
                if HumHeroDB.Open then begin
                  ...
                  if nIndex >= 0 then begin
                    if HumHeroDB.Get(nIndex, HeroNameInfo) >= 0 then begin
                      HeroNameInfo.Header.boDeleted:= True;
                      HumHeroDB.Update(nIndex, HeroNameInfo);
                      sHeroName:= HeroNameInfo.Data.sNewHeroName;
                    end;
                  end;
                end;
                if sHeroName <> '' then FrmUserSoc.UpdateHeroMir(sHeroName, True);
              finally
                HumHeroDB.Close;
              end;
              {$IFEND}*)
...
//--------------------------20080220--------------------------------------------
      if HumDataDB.Open then begin
        ...
        if nIndex >= 0 then begin//20090106
          ...
          HumDataDB.Update(nIndex, HumanRCD);
          {$IFEND}
          if HumanRCD.Data.boHasHero or HumanRCD.Data.boIsHero then begin//有英雄,则把英雄也恢复 20081005
            if HumDataDB.count > 0 then begin//20101012 修改
              ChrList := TStringList.Create;
              try
                if HumChrDB.Open then begin
           ...
           (* {$IF DBSMode = 0}
            try//恢复主体英雄名数据 20100204
              ...
              if sHeroName <> '' then FrmUserSoc.UpdateHeroMir(sHeroName, False);
            finally
              HumHeroDB.Close;
            end;
            {$IFEND}  *)
  ...
  try
    if HumChrDB.Open then begin
      if HumChrDB.ChrCountOfAccount(FrmCreateChr.sUserId) < 2 then begin
        ...
        if (HumRecord.Header.sName <> '') and (HumChrDB.Index(FrmCreateChr.sChrName) = -1) then begin
...
begin
  ...
  sChrName:= ChrGrid.Cells[1, nRow];
  if MessageDlg('是否确认禁用记录 ' + IntToStr(nIndex) + ' ？', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
    try
      if HumChrDB.Open then begin
        if {$IF DBSMode = 1}HumChrDB.GetBy(sChrName, HumRecord){$ELSE}HumChrDB.GetBy(nIndex, HumRecord){$IFEND} then begin
          HumRecord.boDeleted := True;
          HumRecord.dModDate := Now();
          Inc(HumRecord.btCount);
          {$IF DBSMode = 1}
...
begin
  ...
  * Possible String Reference to: '镕&?腱儅?
  |
  ...
  * Possible String Reference to: '儅?
  |
...
begin
  ...
  * Possible String Reference to: '槠%?滕]脣纼-ほJ'
  |
  ...
  * Possible String Reference to: ']脣纼-ほJ'
  |
...
begin
  ...
  if sName = '' then Exit;
  //frmEditRcd.sHeroName1:='';
  try
    if {$IF DBSMode = 1}HumDataDB.Open{$ELSE}HumDataDB.OpenEx{$IFEND} then begin
      nIdx := HumDataDB.Index(sName);
      if nIdx >= 0 then begin
        if {$IF DBSMode = 1}HumDataDB.Get(nIdx, @ChrRecord) >= 0{$ELSE}HumDataDB.Get(nIdx, ChrRecord) >= 0{$IFEND} then begin
          frmEditRcd.m_nIdx := -1;
          frmEditRcd.m_nIdx := nIdx;
          frmEditRcd.HumIndex := StrToInt(ChrGrid.Cells[0, nRow]);//20090407
          ...
          {$IF DBSMode = 1}
          if not ChrRecord.Header.boIsHero then frmEditRcd.sHeroName1:= ChrRecord.Header.sName;
         (* {$ELSE}
          try
            if HumHeroDB.Open then begin
              nIdx := HumHeroDB.Index(sName);
              if nIdx >= 0 then begin
                if HumHeroDB.Get(nIdx, HeroNameInfo) >= 0 then begin
                  frmEditRcd.sHeroName1:= HeroNameInfo.Data.sNewHeroName;
                end;
              end;
            end;
          finally
