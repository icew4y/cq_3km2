...
type
  TFrmMain = class(TForm)
    ...
    bsSkinFileEdit4: TbsSkinFileEdit;
    ...
    procedure bsSkinFileEdit4ButtonClick(Sender: TObject);
...
begin
  ...
  FrmDM.TableStdItems.Active := True;
       ...
       ItemsDBGrid.Columns[24].Title.Caption := 'HP+';
       ItemsDBGrid.Columns[25].Title.Caption := 'MP+';
...
procedure TFrmMain.bsSkinButton8Click(Sender: TObject);
...
begin
  ...
  try
    ...
    with DestTable do begin
      ...
      DataBaseName:= bsSkinFileEdit4.text;//保存路径
      TableName:='NewStdItems';//数据库名
      TableType:=ttParadox;//数据库类型
      CreateTable;//创建新的数据表
    end;

    if (FrmDM.TableStdItems.findfield('Desc') = nil) or (FrmDM.TableStdItems.findfield('HP') = nil) or
       (FrmDM.TableStdItems.findfield('MP') = nil) then
    begin//判断字段是否存在
      ADOA:=TADOQUERY.Create(nil);
      with ADOA do begin
        ConnectionString:=GetParadoxConnectionString(bsSkinFileEdit4.text,'');
        if (FrmDM.TableStdItems.findfield('Desc') = nil) then
        begin
          SQL.Add('ALTER TABLE NewStdItems ADD [Desc] CHAR(80)');
          ExecSQL;
        end;
        if (FrmDM.TableStdItems.findfield('HP') = nil)  then
        begin
          SQL.Clear;
          SQL.Add('ALTER TABLE NewStdItems ADD HP integer');
          ExecSQL;
        end;
        if (FrmDM.TableStdItems.findfield('MP') = nil)  then
        begin
          SQL.Clear;
          SQL.Add('ALTER TABLE NewStdItems ADD MP integer');
          ExecSQL;
        end;
      end;
      ADOA.Free;
    ...
    with FrmDM.Query1 do begin
      ...
      while not FrmDM.Query1.Eof do begin
        with DestTable  do begin
          ...
          FieldByName('Desc').AsString:='0';
          FieldByName('HP').AsInteger:= 0;
          FieldByName('MP').AsInteger:= 0;
          post;
  ...
  bsSkinMessage1.MessageDlg('物品数据库扩展完成！'+bsSkinFileEdit4.text+'\NewStdItems.db',mtInformation,[mbOK],0);
...
begin
  ...
  if Wil.Stream<>nil then begin
      ...
      case WIl.FileType of
       ...
       End;
       3: LabelType.Caption:='WZL 格式';   //Zlib
      end;
...
begin                                               //Zlib
  OpenDialog1.Filter := 'Wil文件(*.Wil)|*.Wil|Wzl文件(*.Wzl)|*.Wzl';
  ...
  if OpenDialog1.Execute then begin
    ...
    if FileExists(EditFileName.Text) then begin
      ...
      Wil.Initialize;
      if wil.Stream= nil then begin
        if Wil.boIsZlibFormat then
          bsSkinMessage1.MessageDlg('WZl文件已经被破坏或不是wzl文件！',mtInformation,[mbOK],0)
        else
...
procedure TFrmMain.bsSkinFileEdit4ButtonClick(Sender: TObject);
begin
  if bsSkinSelectDirectoryDialog1.Execute then begin
    bsSkinFileEdit4.Text:= bsSkinSelectDirectoryDialog1.Directory;
    if bsSkinFileEdit4.Text <> '' then bsSkinButton8.Enabled:= True;
  end;
end;

procedure TFrmMain.bsSkinPageControl1Change(Sender: TObject);
begin
  if (bsSkinTabSheet1 <> bsSkinPageControl1.ActivePage) or (frmlogin <> nil) then Exit;
