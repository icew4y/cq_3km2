...
begin
  ...
  bo11 := False;
  try
    if {$IF IDMODE = 1}AccountDB.Open{$ELSE}AccountDB.OpenEx{$IFEND} then begin
...
begin
    IdGrid.Cells[0, nRow1] := '3KM2';
    IdGrid.Cells[1, nRow1] := '3KM2';
    IdGrid.Cells[2, nRow1] := '3KM2';
  ...
  for I := 1 to IdGrid.RowCount - 1  do begin
    try
      if NewIDDB.Open then begin
        ...
        DBRecord.Header.CreateDate:= {StrToDateTime('1899-12-30')}Now() ;//20091219 修改
        DBRecord.Header.UpdateDate := {StrToDateTime('2008-8-1 20:00:00')}Now();//20091219 修改
