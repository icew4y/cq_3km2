    ...
    if MsgServer.Socket = Socket then begin
      ...
      while (Pos(')', sReviceMsg) > 0) do begin
        ...
        case nCode of
          ...
          SS_LOGINCOST: begin//接收M2发来充值账号的已使用时间
              sMsg := GetValidStr3(sMsg, sAccount, ['/']);
              nC:= Str_ToInt(sMsg, 0);
              nIndex := Config.AccountCostList.IndexOf(sAccount);
              if nIndex > -1 then begin
                AccountCostRcd := pTAccountCostRcd(Config.AccountCostList.Objects[nIndex]);
                if (AccountCostRcd <> nil) and (nC > 0) then begin
                  if AccountCostRcd.nCostType = 0 then begin//模式为小时时
                    AccountCostRcd.nAccountCost:= _MAX(0, AccountCostRcd.nAccountCost - nC);
                    if AccountCostRcd.nAccountCost <= 0 then begin
                      AccountCostRcd.boEnable:= False;
                      SessionKick(Config, sAccount, True);
                    end;
                  end;
                end;
              end else SessionKick(Config, sAccount, True);
            end;
