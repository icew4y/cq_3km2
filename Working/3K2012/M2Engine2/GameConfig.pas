...
uses
  Windows, SysUtils, Classes, Controls, Forms, Grobal2, StdCtrls, Grids,
  Spin, ComCtrls;
...
type
  ...
  TfrmGameConfig = class(TForm)
    ...
    CheckBoxSaveHighLevelKillMonFixExp: TCheckBox;
    ...
    LabelHearMsgFColor1: TLabel;
    EditHearMsgFColor1: TSpinEdit;
    Label151: TLabel;
    Label152: TLabel;
    EditHearMsgFColor2: TSpinEdit;
    LabelHearMsgFColor2: TLabel;
    Label154: TLabel;
    EditHearMsgFColor3: TSpinEdit;
    LabelHearMsgFColor3: TLabel;
    Label150: TLabel;
    SpinEditEatHPItems: TSpinEdit;
    GridLevelToExpRate: TStringGrid;
    CheckBoxPKHeroDie: TCheckBox;
    GroupBox78: TGroupBox;
    Label153: TLabel;
    Label155: TLabel;
    LabelSayMsgFColor1: TLabel;
    LabelSayMsgBColor1: TLabel;
    EditSayMsgFColor1: TSpinEdit;
    EditSayeMsgBColor1: TSpinEdit;
    CheckBoxLevelLessNotIncPKPoint: TCheckBox;
    SpinEditNotIncPKPointLevel: TSpinEdit;
    GroupBox82: TGroupBox;
    CheckBoxCanHero: TCheckBox;
    CheckBoxContinuousUpLevel: TCheckBox;
    GroupBox76: TGroupBox;
    Label148: TLabel;
    EditDiyMedicine: TEdit;
    SpinEditLimitExpLevelHero: TSpinEdit;
    Label160: TLabel;
    CheckNoCanUseProtection: TCheckBox;
    CheckNoCanUseHeroProtection: TCheckBox;
    CheckBoxItmeAutoOver: TCheckBox;
    GroupBox83: TGroupBox;
    Label161: TLabel;
    Label162: TLabel;
    LabelItemMsgFColor: TLabel;
    LabelItemMsgBColor: TLabel;
    EditItemMsgFColor: TSpinEdit;
    EditItemMsgBColor: TSpinEdit;
    CheckCanUseComparThrust: TCheckBox;
    GroupBox84: TGroupBox;
    Label163: TLabel;
    Label164: TLabel;
    LabelDivisionMsgFColor: TLabel;
    LabelDivisionMsgBColor: TLabel;
    EditDivisionMsgFColor: TSpinEdit;
    EditDivisionMsgBColor: TSpinEdit;
    CheckBoxCanDivision: TCheckBox;
    cbxShowMoveHP: TCheckBox;
    cbxShowMoveNewItem: TCheckBox;
    cbxUses176StateWin: TCheckBox;
    ...
    procedure CheckBoxSaveHighLevelKillMonFixExpClick(Sender: TObject);
    procedure EditHearMsgFColor1Change(Sender: TObject);
    procedure EditHearMsgFColor2Change(Sender: TObject);
    procedure EditHearMsgFColor3Change(Sender: TObject);
    procedure SpinEditEatHPItemsChange(Sender: TObject);
    procedure CheckBoxPKHeroDieClick(Sender: TObject);
    procedure EditSayMsgFColor1Change(Sender: TObject);
    procedure EditSayeMsgBColor1Change(Sender: TObject);
    procedure CheckBoxLevelLessNotIncPKPointClick(Sender: TObject);
    procedure SpinEditNotIncPKPointLevelChange(Sender: TObject);
    procedure CheckBoxCanHeroClick(Sender: TObject);
    procedure CheckBoxContinuousUpLevelClick(Sender: TObject);
    procedure EditDiyMedicineChange(Sender: TObject);
    procedure SpinEditLimitExpLevelHeroChange(Sender: TObject);
    procedure CheckNoCanUseProtectionClick(Sender: TObject);
    procedure CheckNoCanUseHeroProtectionClick(Sender: TObject);
    procedure CheckBoxItmeAutoOverClick(Sender: TObject);
    procedure EditItemMsgFColorChange(Sender: TObject);
    procedure EditItemMsgBColorChange(Sender: TObject);
    procedure CheckCanUseComparThrustClick(Sender: TObject);
    procedure EditDivisionMsgFColorChange(Sender: TObject);
    procedure EditDivisionMsgBColorChange(Sender: TObject);
    procedure CheckBoxCanDivisionClick(Sender: TObject);
    procedure cbxShowMoveHPClick(Sender: TObject);
    procedure cbxShowMoveNewItemClick(Sender: TObject);
    procedure cbxUses176StateWinClick(Sender: TObject);
  private
    boOpened: Boolean;
    boModValued: Boolean;
    procedure ModValue();
...
begin
  ...
  GridLevelToExpRate.Cells[0, 0] := '等级';
  GridLevelToExpRate.Cells[1, 0] := '比例(%)';
  for I := 1 to GridLevelToExpRate.RowCount - 1 do begin
    GridLevelToExpRate.Cells[0, I] := IntToStr(I);
  end;
  ...
  GridLevelExp.Hint := '修改的经验在点击保存按钮后生效。经验值最高4294967295';
...
{$IF HEROVERSION <> 2}
  ...
  CheckBoxCanHero.Checked := g_Config.boUseCanHero;
  CheckBoxCanDivision.Checked := g_Config.boUseCanDivision; // 开放师门系统

  CheckBoxPKHeroDie.Visible := True;
  SpinEditLimitExpLevelHero.Visible := True;
  Label160.Visible := True;
  Label146.Top:= 12;
  SpinEditLimitExpLevel.Top:= 10;
  CheckNoCanUseHeroProtection.Visible := True;
  CheckCanUseComparThrust.left:= 204;
{$IFEND}
  ...
  CheckBoxSaveHighLevelKillMonFixExp.Checked := g_Config.boSaveHighLevelKillMonFixExp;//是否保存真视经验时间 20090213
  CheckBoxContinuousUpLevel.Checked := g_Config.boContinuousUpLevel;//是否使用连续升级
  ...
  for I := 1 to GridLevelToExpRate.RowCount - 1 do begin
    GridLevelToExpRate.Cells[1, I] := IntToStr(g_Config.dwLevelToExpRate[I]);
  end;
  (*
  ...
  *)
  ...
  CheckBoxItmeAutoOver.Checked := g_Config.boItmeAutoOver;
  cbxShowMoveHP.Checked := g_Config.boShowMoveHP;
  cbxShowMoveNewItem.Visible := M2Version = 2;
  {$If M2Version = 2}
  cbxShowMoveNewItem.Checked := g_Config.boShowNewItem;
  {$Ifend}
  cbxUses176StateWin.Visible := M2Version = 0;
  {$If M2Version = 0}
  cbxUses176StateWin.Checked := g_Config.boUses176StateWin;
  {$Ifend}
  CheckNoCanUseProtection.Checked := g_Config.boNoCanUseProtection;
  CheckNoCanUseHeroProtection.Checked := g_Config.boNoCanUseHeroProtection;
  CheckCanUseComparThrust.Checked := g_Config.boNoCanUseComparThrust;
  ...
  EditDiyMedicine.Text:= g_Config.sDiyMedicine;
  ...
  EditHearMsgFColor1.Value := g_Config.btHearMsgFColor1;
  EditHearMsgFColor2.Value := g_Config.btHearMsgFColor2;
  EditHearMsgFColor3.Value := g_Config.btHearMsgFColor3;
  ...
  EditSayMsgFColor1.Value := g_Config.btSayMsgFColor1; //前景 传音筒
  EditSayeMsgBColor1.Value := g_Config.btSayeMsgBColor1;//背景 传音筒
{$IF M2Version <> 2}

  EditDivisionMsgFColor.Value := g_Config.btDivisionMsgFColor;
  EditDivisionMsgBColor.Value := g_Config.btDivisionMsgBColor;
{$IFEND}
  EditItemMsgFColor.Value := g_Config.btItemMsgFColor;
  EditItemMsgBColor.Value := g_Config. btItemMsgBColor;
  ...
  CheckBoxLevelLessNotIncPKPoint.Checked := g_Config.boLevelLessNotIncPKPoint;
  SpinEditNotIncPKPointLevel.Value := g_Config.nNotIncPKPointLevel;
  CheckBoxLevelLessNotIncPKPointClick(CheckBoxLevelLessNotIncPKPoint);
  CheckBoxPKLevelProtect.Checked := g_Config.boPKLevelProtect;
  CheckBoxPKHeroDie.Checked := g_Config.boPKHeroDieAddBodyLuck;//杀英雄增加诅咒值 20090414
  ...
  {$IF HEROVERSION = 1}
  SpinEditLimitExpLevelHero.Value := g_Config.nLimitExpLevelHero;
  {$IFEND}
  ...
  //内挂结束
  TabSheet10.TabVisible:= False;//隐藏状态控制页面 20091031
  RefGameVarConf();
  RefCharStatusConf();
  boOpened := True;
...
begin
  ...
  SpinEditEatHPItems.Value := g_Config.dwEatHPItemsIntervalTime;//吃药间隔 20090301
  EditTurnIntervalTime.Value := g_Config.dwTurnIntervalTime;

  EditItemSpeedTime.Value := g_ClientConf.btItemSpeed;
...
{$IF SoftVersion <> VERDEMO}
  ...
  Config.WriteInteger('Setup', 'EatHPItemsIntervalTime', g_Config.dwEatHPItemsIntervalTime);//吃药间隔 20090301
...
begin
  if not boOpened then Exit;
  g_ClientConf.btItemSpeed := EditItemSpeedTime.Value;
  ModValue();
end;
...
procedure TfrmGameConfig.cbxShowMoveHPClick(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.boShowMoveHP := cbxShowMoveHP.Checked;
  ModValue();
end;

procedure TfrmGameConfig.cbxShowMoveNewItemClick(Sender: TObject);
begin
  {$If M2Version = 2}
  if not boOpened then Exit;
  g_Config.boShowNewItem := cbxShowMoveNewItem.Checked;
  ModValue();
  {$Ifend}
end;

procedure TfrmGameConfig.cbxUses176StateWinClick(Sender: TObject);
begin
  {$If M2Version = 0}
  if not boOpened then Exit;
  g_Config.boUses176StateWin := cbxUses176StateWin.Checked;
  ModValue();
  {$Ifend}
end;
...
procedure TfrmGameConfig.EditItemMsgBColorChange(Sender: TObject);
var
  btColor: Byte;
begin
  btColor := EditItemMsgBColor.Value;
  LabelItemMsgBColor.Color := GetRGB(btColor);
  if not boOpened then Exit;
  g_Config.btItemMsgBColor := btColor;
  ModValue();
end;

procedure TfrmGameConfig.EditItemMsgFColorChange(Sender: TObject);
var
  btColor: Byte;
begin
  btColor := EditItemMsgFColor.Value;
  LabelItemMsgFColor.Color := GetRGB(btColor);
  if not boOpened then Exit;
  g_Config.btItemMsgFColor := btColor;
  ModValue();
end;
...
var
  ...
  LevelToExpRate: TLevelNeedExp3;
begin
  for I := 1 to GridLevelExp.RowCount - 1 do begin
    dwExp := Str_ToInt(GridLevelExp.Cells[1, I], 0);
    if (dwExp <= 0) or (dwExp > High(UInt64)) then begin
  ...
  for I := 1 to GridLevelToExpRate.RowCount - 1 do begin
    dwExp := Str_ToInt(GridLevelToExpRate.Cells[1, I], 0);
    if (dwExp <= 0) or (dwExp > 100) then begin
      Application.MessageBox(PChar('等级 ' + IntToStr(I) + ' 比例设置错误(不能小于0或大于100)！'), '错误信息', MB_OK + MB_ICONERROR);
      GridLevelToExpRate.Row := I;
      GridLevelToExpRate.SetFocus;
      Exit;
    end;
    LevelToExpRate[I] := dwExp;
  end;
  g_Config.dwLevelToExpRate:= LevelToExpRate;
  ...
  Config.WriteBool('Exp', 'SaveHighLevelKillMonFixExp', g_Config.boSaveHighLevelKillMonFixExp);//是否保存真视经验时间 20090213
  ...
  for I := 1 to 1000 do begin
    ...
    Config.WriteString('LevelToExpRate', 'Level' + IntToStr(I), IntToStr(g_Config.dwLevelToExpRate[I]));
  end;
...
procedure TfrmGameConfig.CheckCanUseComparThrustClick(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.boNoCanUseComparThrust := CheckCanUseComparThrust.Checked;
  ModValue();
end;

procedure TfrmGameConfig.CheckNoCanUseHeroProtectionClick(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.boNoCanUseHeroProtection := CheckNoCanUseHeroProtection.Checked;
  ModValue();
end;

procedure TfrmGameConfig.CheckNoCanUseProtectionClick(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.boNoCanUseProtection := CheckNoCanUseProtection.Checked;
  ModValue();
end;
...
procedure TfrmGameConfig.CheckBoxItmeAutoOverClick(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.boItmeAutoOver := CheckBoxItmeAutoOver.Checked;
  ModValue();
end;
...
begin
  if EditDiyMedicine.text <> '' then begin
    if TagCount(EditDiyMedicine.text,'|') <= 0 then begin
      Application.MessageBox('自定义药品输入不正确,格式:XXX|XXX|！！！', '提示信息', MB_OK + MB_ICONERROR);
      EditDiyMedicine.SetFocus;
      Exit;
    end;
    if TagCount(EditDiyMedicine.text,'|') > 5 then begin
      Application.MessageBox('自定义药品输入不正确,只能支持5个自定义药品！！！', '提示信息', MB_OK + MB_ICONERROR);
      EditDiyMedicine.SetFocus;
      Exit;
    end;
    g_Config.sDiyMedicine := EditDiyMedicine.Text;
  end;
{$IF SoftVersion <> VERDEMO}
...
{$IF SoftVersion <> VERDEMO}
  ...
  Config.WriteBool('Setup', 'LevelLessNotIncPKPoint', g_Config.boLevelLessNotIncPKPoint);//等级小于指定值不增加PK点
  Config.WriteInteger('Setup', 'NotIncPKPointLevel', g_Config.nNotIncPKPointLevel);//等级小于时，不增加PK点
  
  Config.WriteBool('Setup', 'PKProtect', g_Config.boPKLevelProtect);
  Config.WriteBool('Setup', 'PKHeroDieAddBodyLuck', g_Config.boPKHeroDieAddBodyLuck);//杀英雄增加诅咒值 20090414
...
procedure TfrmGameConfig.EditDivisionMsgBColorChange(Sender: TObject);
var
  btColor: Byte;
begin
  {$IF M2Version <> 2}
  btColor := EditDivisionMsgBColor.Value;
  LabelDivisionMsgBColor.Color := GetRGB(btColor);
  if not boOpened then Exit;
  g_Config.btDivisionMsgBColor := btColor;
  ModValue();
  {$IFEND}
end;

procedure TfrmGameConfig.EditDivisionMsgFColorChange(Sender: TObject);
var
  btColor: Byte;
begin
  {$IF M2Version <> 2}
  btColor := EditDivisionMsgFColor.Value;
  LabelDivisionMsgFColor.Color := GetRGB(btColor);
  if not boOpened then Exit;
  g_Config.btDivisionMsgFColor := btColor;
  ModValue();
  {$IFEND}
end;

procedure TfrmGameConfig.EditDiyMedicineChange(Sender: TObject);
begin
  if not boOpened then Exit;
  ModValue();
end;
...
{$IF SoftVersion <> VERDEMO}
  ...
  Config.WriteInteger('Setup', 'SayMsgFColor1', g_Config.btSayMsgFColor1);//前景 传音筒
  Config.WriteInteger('Setup', 'SayeMsgBColor1', g_Config.btSayeMsgBColor1);//背景 传音筒
{$IF M2Version <> 2}
...
begin
  CheckBoxParalyCanRun.Checked := g_ClientConf.boParalyCanRun;
  CheckBoxParalyCanWalk.Checked := g_ClientConf.boParalyCanWalk;
  CheckBoxParalyCanHit.Checked := g_ClientConf.boParalyCanHit;
  CheckBoxParalyCanSpell.Checked := g_ClientConf.boParalyCanSpell;
end;
...
begin
  if not boOpened then Exit;
  g_ClientConf.boParalyCanRun := CheckBoxParalyCanRun.Checked;
  ModValue();
end;
...
begin
  if not boOpened then Exit;
  g_ClientConf.boParalyCanWalk := CheckBoxParalyCanWalk.Checked;
  ModValue();
end;
...
begin
  if not boOpened then Exit;
  g_ClientConf.boParalyCanHit := CheckBoxParalyCanHit.Checked;
  ModValue();
end;
...
begin
  if not boOpened then Exit;
  g_ClientConf.boParalyCanSpell := CheckBoxParalyCanSpell.Checked;
  ModValue();
end;
...
procedure TfrmGameConfig.SpinEditLimitExpLevelHeroChange(Sender: TObject);
begin
{$IF HEROVERSION = 1}
  if not boOpened then Exit;
  g_Config.nLimitExpLevelHero := SpinEditLimitExpLevelHero.Value;
  ModValue();
  {$IFEND}
end;
...
begin
(*{$IF SoftVersion <> VERDEMO}
 Config.WriteInteger('WgInfo', 'ClientWgInfo', g_ClientConf.nClientWgInfo);
  Config.WriteBool('WgInfo', 'ShowRedHPLable', g_ClientConf.boShowRedHPLable);
  Config.WriteBool('WgInfo', 'ShowGroupMember', g_ClientConf.boShowGroupMember);
  Config.WriteBool('WgInfo', 'ShowAllItem', g_ClientConf.boShowAllItem);
  Config.WriteBool('WgInfo', 'ShowBlueMpLable', g_ClientConf.boShowBlueMpLable);
  Config.WriteBool('WgInfo', 'ShowName', g_ClientConf.boShowName);
  Config.WriteBool('WgInfo', 'AutoPuckUpItem', g_ClientConf.boAutoPuckUpItem);
  Config.WriteBool('WgInfo', 'ShowHPNumber', g_ClientConf.boShowHPNumber);
  Config.WriteBool('WgInfo', 'ShowAllName', g_ClientConf.boShowAllName);
  Config.WriteBool('WgInfo', 'ForceNotViewFog', g_ClientConf.boForceNotViewFog);
  Config.WriteBool('WgInfo', 'ParalyCan', g_ClientConf.boParalyCan);
  Config.WriteBool('WgInfo', 'MoveSlow', g_ClientConf.boMoveSlow);
  Config.WriteBool('WgInfo', 'CanStartRun', g_ClientConf.boCanStartRun);
  Config.WriteBool('WgInfo', 'AutoMagic', g_ClientConf.boAutoMagic);
  Config.WriteBool('WgInfo', 'MoveRedShow', g_ClientConf.boMoveRedShow);
  Config.WriteBool('WgInfo', 'MagicLock', g_ClientConf.boMagicLock);
  Config.WriteInteger('WgInfo', 'MoveTime', g_ClientConf.nMoveTime);
  Config.WriteInteger('WgInfo', 'HitTime', g_ClientConf.nHitTime);
  Config.WriteInteger('WgInfo', 'SpellTime', g_ClientConf.nSpellTime);
{$IFEND}
  uModValue();*)
end;
...
procedure TfrmGameConfig.CheckBoxSaveHighLevelKillMonFixExpClick(
  Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.boSaveHighLevelKillMonFixExp := CheckBoxSaveHighLevelKillMonFixExp.Checked;
  ModValue();
end;

procedure TfrmGameConfig.EditHearMsgFColor1Change(Sender: TObject);
var
  btColor: Byte;
begin
  btColor := EditHearMsgFColor1.Value;
  LabelHearMsgFColor1.Color := GetRGB(btColor);
  if not boOpened then Exit;
  g_Config.btHearMsgFColor1 := btColor;
  ModValue();
end;

procedure TfrmGameConfig.EditHearMsgFColor2Change(Sender: TObject);
var
  btColor: Byte;
begin
  btColor := EditHearMsgFColor2.Value;
  LabelHearMsgFColor2.Color := GetRGB(btColor);
  if not boOpened then Exit;
  g_Config.btHearMsgFColor2 := btColor;
  ModValue();
end;

procedure TfrmGameConfig.EditHearMsgFColor3Change(Sender: TObject);
var
  btColor: Byte;
begin
  btColor := EditHearMsgFColor3.Value;
  LabelHearMsgFColor3.Color := GetRGB(btColor);
  if not boOpened then Exit;
  g_Config.btHearMsgFColor3 := btColor;
  ModValue();
end;

//吃药间隔 20090301
procedure TfrmGameConfig.SpinEditEatHPItemsChange(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.dwEatHPItemsIntervalTime := SpinEditEatHPItems.Value;
  ModValue();
end;

procedure TfrmGameConfig.CheckBoxPKHeroDieClick(Sender: TObject);
var
  boStatus: Boolean;
begin
  boStatus := CheckBoxPKHeroDie.Checked;
  if not boOpened then Exit;
  g_Config.boPKHeroDieAddBodyLuck := boStatus;
  ModValue();
end;

procedure TfrmGameConfig.EditSayMsgFColor1Change(Sender: TObject);
var
  btColor: Byte;
begin

  btColor := EditSayMsgFColor1.Value;
  LabelSayMsgFColor1.Color := GetRGB(btColor);
  if not boOpened then Exit;
  g_Config.btSayMsgFColor1 := btColor;
  ModValue();

end;

procedure TfrmGameConfig.EditSayeMsgBColor1Change(Sender: TObject);
var
  btColor: Byte;
begin

  btColor := EditSayeMsgBColor1.Value;
  LabelSayMsgBColor1.Color := GetRGB(btColor);
  if not boOpened then Exit;
  g_Config.btSayeMsgBColor1 := btColor;
  ModValue();

end;

procedure TfrmGameConfig.CheckBoxLevelLessNotIncPKPointClick(
  Sender: TObject);
var
  boStatus: Boolean;
begin
  boStatus := CheckBoxLevelLessNotIncPKPoint.Checked;
  SpinEditNotIncPKPointLevel.Enabled := boStatus;
  if not boOpened then Exit;
  g_Config.boLevelLessNotIncPKPoint := boStatus;
  ModValue();
end;

procedure TfrmGameConfig.SpinEditNotIncPKPointLevelChange(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.nNotIncPKPointLevel := SpinEditNotIncPKPointLevel.Value;
  ModValue();
end;

procedure TfrmGameConfig.CheckBoxCanDivisionClick(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.boUseCanDivision := CheckBoxCanDivision.Checked;
  ModValue();
end;

procedure TfrmGameConfig.CheckBoxCanHeroClick(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.boUseCanHero := CheckBoxCanHero.Checked;
  ModValue();
end;

procedure TfrmGameConfig.CheckBoxContinuousUpLevelClick(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.boContinuousUpLevel := CheckBoxContinuousUpLevel.Checked;
  ModValue();
end;
