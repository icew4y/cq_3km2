...
begin
  if boEnterKey then begin
    ...
    case btUserMode of
      1: wCount :={ CountSpinEdit.Value}High(word);
...
{$ELSEIF Version = UserKey1}
      nVersion := UserKey1 * Integer(boEnterKey);
{$ELSEIF Version = UserKey2}
      nVersion := UserKey2 * Integer(boEnterKey);
{$ELSEIF Version = UserKey3}
      nVersion := UserKey3 * Integer(boEnterKey);
{$ELSEIF Version = UserKey4}
      nVersion := UserKey4 * Integer(boEnterKey);
{$ELSEIF Version = UserKey5}
      nVersion := UserKey5 * Integer(boEnterKey);
{$ELSEIF Version = UserKey6}
      nVersion := UserKey6 * Integer(boEnterKey);
{$IFEND}
